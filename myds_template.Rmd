...
if (glb_is_classification)
    print(myplot_scatter(plot_models_df, "AIC.fit", "f.score.OOB") + 
          geom_text(aes(label=feats.label), data=plot_models_df, color="NavyBlue", 
                    size=3.5, angle=45))
...
if (glb_is_classification) {
    ...
    glb_sel_mdl <- glb_mdl        
    glb_entity_df[, paste0(glb_predct_var_name, ".proba")] <- predict(glb_sel_mdl, 
                        newdata=glb_entity_df, type="response")
    for (clf_threshold in c(0.4, 0.5, 0.6))
    ...
    glb_entity_df[, glb_predct_var_name] <- 
        (glb_entity_df[, paste0(glb_predct_var_name, ".proba")] >= 0.5) * 1.0
