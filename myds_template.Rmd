...
glbMdlPreprocMethods <- NULL # : default # list(
...
# ```{r cluster.data, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c("glbFeatsCluster", "glbFeatsClusterVarsExclude"), dropSpec = c("glb_cluster", "glb_cluster_entropy_var", "glbFeatsTextClusterVarsExclude"))}
#myloadChunk(glbChunks$inpFilePathName, keepSpec = c("glbFeatsCluster", "glbFeatsClusterVarsExclude"), dropSpec = c("glb_cluster", "glb_cluster_entropy_var", "glbFeatsTextClusterVarsExclude"))
#keepSpec = c("glbFeatsCategory","glb_dsp_cols")) # if glbFeatsCategory needs to be changed
...
#             as.character(rspvar_freq_df[, glb_rsp_var_raw])
        ...
        split_strata <- sampling::strata(tmp_trnobs_df, 
                               ...
                               pik = NewTrnSmy$cosineSmy, method = "systematic")        
    ...
    for (metric in names(smyDf)[!grepl("obs.", names(smyDf), fixed = TRUE)])
        stopifnot(smyDf[(smyDf$obs.x == "OOB") & (smyDf$obs.y == "New"), metric] >= 
                  smyDf[(smyDf$obs.x == "Fit") & (smyDf$obs.y == "New"), metric])
} else {
...
#```{r select.features, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c("glbMdlAllowParallel", "glbMdlSelId"), dropSpec = c("glb_preproc_methods"))}
#myloadChunk(glbChunks$inpFilePathName, keepSpec = c("glbMdlAllowParallel", "glbMdlSelId"), dropSpec = c("glb_preproc_methods"))
...
#```{r fit.models_1, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c("glbMdlFamilies", "glbRFESizes", "glbMdlAllowParallel","glbMdlPreprocMethods"), dropSpec = c(NULL))}
#myloadChunk(glbChunks$inpFilePathName, keepSpec = c("glbMdlFamilies", "glbRFESizes", "glbMdlAllowParallel","glbMdlPreprocMethods"), dropSpec = c(NULL)); glb2Sav()
...
if ((any(grepl("RFE.X", names(glbMdlFamilies), fixed = TRUE))) &&
    ...
    indepVar <- mygetIndepVar(glb_feats_df)
    glbRFEResults <- myrun_rfe(obs_df = glbObsFit, indepVar = indepVar, 
...
#     } else fitobs_df <- glbObsFit
#     
#     for (mdlPrepr in glbMdlPreprocMethods[[mdlFamily]][[mdlAlgMethod]]) {
#         # for (mdlPrepr in glbMdlPreprocMethods[[mdlFamily]][[mdlAlgMethod]][-(1:13)]) {
#         ret_lst <- myfit_mdl(mdl_specs_lst = myinit_mdl_specs_lst(mdl_specs_lst = list(
...
#```{r fit.models_2, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c("glbMdlAllowParallel", "glbMdlEnsemble", "glbMdlMetricsEval", "glbMdlTuneParams"), dropSpec = c(NULL))}
#myloadChunk(glbChunks$inpFilePathName, keepSpec = c("glbMdlAllowParallel", "glbMdlEnsemble", "glbMdlMetricsEval", "glbMdlTuneParams"), dropSpec = c(NULL)); glb2Sav()
...
#         indepVar <- indepVar[!grepl("err$", indepVar)]
    ...
    # print(head(bstMdlDf, 5))
    bstMdlId <- bstMdlDf[1, "id"]
    # bstIndepVar <- paste0(glb_rsp_var, ".", gsub("#", ".", bstMdlId, fixed = TRUE))
