...
# splitSpecs = list (method <- # select from c(NULL, "condition", "sample", "copy"))
...
#orderBy(~term, glb_post_stem_words_terms_df_lst[[txtFeat]][grep("^qua", glb_post_stem_words_terms_df_lst[[txtFeat]]$term), ])
#glbObsAll[glb_post_stem_words_terms_mtrx_lst[[txtFeat]][, 8640] > 0, glbFeatsText]
...
#paste0(sort(setdiff(subset(glb_post_stem_words_terms_df_lst[[txtFeat]], (nzv.freqRatio >= glb_nzv_freqCut) & (freq < 10) & (chisq.pval >= 0.05))$term, c( "128gb","3g","4g","gold","ipad1","ipad3","ipad4","ipadair2","ipadmini2","manufactur","spacegray","sprint","tmobil","verizon","wifion"))), collapse=",")
...
glbFeatsTextVldWords <- myreplacePunctuation(str_to_lower(gsub(" ", "", c(NULL
        # custom names/terms for names screening
                                    ))))
...
#glbObsAll[which(terms_stem_mtrx[, grep("use", dimnames(terms_stem_mtrx)$Terms)[[1]]] > 0), c(glb_id_var, "productline", txtFeat)]
#glbObsAll[which(TfIdf_stem_mtrx[, 191] > 0), c(glb_id_var, glbFeatsCategory, txtFeat)]
#glbObsAll[which(glb_post_stop_words_terms_mtrx_lst[[txtFeat]][, 6165] > 0), c(glb_id_var, glbFeatsCategory, txtFeat)]
...
# list parsed to collect vldWords
...
#dsp.equal(86, 90)
    ...
    glb_txt_chr_lst <- foreach(txtFeat = glbFeatsText) %dopar% {   
...
#     }
    ...
    # This should be run after glbFeatsTextCorpus is created with tolower
#     extract.features_chunk_df <- myadd_chunk(extract.features_chunk_df, 
#             paste0("extract.features_", "process.text_reporting_compound_terms"), major.inc=FALSE)
        
#     for (txtFeat in glbFeatsText) {
#         print(sprintf("Remaining compound terms in %s: ", txtFeat))        
#         find_cmpnd_wrds(txt_vctr = glb_txt_chr_lst[[txtFeat]])
#         #grep("thirty-five", txt_vctr, ignore.case=TRUE, value=TRUE)
#         #rex_str <- glbFeatsTextMap[grepl("hirty", glbFeatsTextMap$rex_str), "rex_str"]
#     }

    extract.features_chunk_df <- myadd_chunk(extract.features_chunk_df, 
            paste0("extract.features_", "build.corpus"), major.inc = TRUE)
    
    get_txt_terms <- function(terms_TDM, compute.chisq = TRUE, compute.nzv = TRUE) {
...
#                                 use = "pairwise.complete.obs"))
        ...
        if (compute.nzv) {
            nzv_df <- 
#                 caret::nzv(docms_mtrx[glbObsAll$.src == "Train",], freqCut = glb_nzv_freqCut,
#                             uniqueCut = glb_nzv_uniqueCut, saveMetrics = TRUE)
                ...
                mycaret.nearZeroVar(docms_mtrx[glbObsAll$.src == "Train",],
                                   freqCut = glb_nzv_freqCut, uniqueCut = glb_nzv_uniqueCut,
...
#stop(here"); glb2Sav()    
    ...
    glbFeatsTextCorpus <- foreach(txtFeat = glbFeatsText, .verbose = FALSE) %dopar% {
...
#         txt-corpus <- tm_map(txt_corpus, content_transformer(function(x, pattern) gsub(pattern, "", x))   
    ...
    names(glbFeatsTextCorpus) <- glbFeatsText
...
mytokenize3grams <- content_transformer(function(x) { 
    ...
    vldWordsFilename <- paste0("scowl-2015.08.24-english-american-35.txt")
    if (!file.exists(vldWordsFilename))
        stop(vldWordsFilename, " not found!")
    vldWords <- read.csv(vldWordsFilename, header = FALSE, comment.char = "#", strip.white = TRUE,
                         encoding = "UTF-8")
    vldWords <- iconv(vldWords[, 1], "latin1", "UTF-8")
    vldWords <- myreplacePunctuation(vldWords)
    vldWords <- vldWords[!duplicated(vldWords)]
    ...
    vldWords <- union(vldWords, txtMapRplStr)
    
                    # Add "rpl_str" from glbFeatsDerive[[txtFeat]] (that don't contain \\#)
    for (feat in intersect(glbFeatsText, names(glbFeatsDerive))) {
        userDefWords <- deparse(glbFeatsDerive[[feat]]$mapfn, width.cutoff = 100)
        userDefWords <- userDefWords[grepl("gsub", userDefWords)]
        userDefWords <- gsub("modRaw <- gsub\\(\"(.+)\", \"(.+)\", (.+)\\)", "\\2", userDefWords)
        #            regexpr("modRaw <- gsub\\(\"(.+)\", \"(.+)\", ",         userDefWords[111])
        userDefWords <- str_trim(gsub("modRaw (.+)", "\\1", userDefWords))
        userDefWords <- userDefWords[!grepl("\\\\", userDefWords)]
        userDefWords <- unique(str_to_lower(unlist(str_split(userDefWords, " "))))
        vldWords <- union(vldWords, userDefWords)
    }    
                        
                    # Add synonyms from glbFeatsTextSynonyms[[txtFeat]]
    for (feat in intersect(glbFeatsText, names(glbFeatsTextSynonyms))) {
        userDefWords <- unlist(sapply(1:length(glbFeatsTextSynonyms[[feat]]), function(elemIx)
                                                    glbFeatsTextSynonyms[[feat]][[elemIx]]$syns))
        vldWords <- union(vldWords, userDefWords)
    }    
                        
    vldWords <- union(vldWords, myreplacePunctuation(str_to_lower(c(NULL
        # Month names
        ...
        ,"Baja","beijing","Belgian","Belgians","belgium","benghazi","Berlin","boston"
            ,"Brazil","Brazilian","britain","british","Britons","broadway","bronx","brooklyn"
            ,"Burma"
        ,"Canada","canadian","canadians","caribbean"
            ...
            ,"Cyprus"
        ,"dallas","Danish","Denmark","detroit","Deutsche","Dutch"
        ...
        ,"Haiti","Haitian","havana","Hawaii","Hawaiian","Helsinki","Hertz","Hispanic"
        ...
        ,"Ibiza","Iceland","illinois","india","indias","indian","indians","iowa"
            ...
            ,"Italia","Italian"
        ,"japan","japanese"
        ,"kansas","kentucky","Kenya","Kenyans","Kiev"
        ...
        ,"Wales","wallstreet's","washington","WashingtonDC","wisconsin"
        ,"Xinjiang"
        ,"Zhejiang"
        
        # Company names
        ,"AbbVie","AbbVie's","Actavis","adidas","Aldermore","Airbnb"
            ,"Alibaba","Alibabas","allergan","Altice","Altuzarra"
            ,"Amazon","Amgen"
            ,"Apax","APEC","Arista","Atos","Auxilium","Aviva"
        ,"Bain","Balenciaga","Balenciaga","barclays","Bayer","BBC","Bellevue","BGC"
            ,"BlackRock","Blackstone","Blumarine"
            ,"Braun"
            ,"BSkyB","bt","Burberry"
        ,"Caesars","Calpers","Cargill","Cartier","CGG"
            ,"Chanel","Chanel's","Chiquita","Chiquita's","Chloe"
            ...
            ,"csx"
            ,"cubist"
        ,"darden","dealogic","Diageo","Dow","DreamWorks","Dresser-Rand"
        ,"EBay","EE","Electra","Elliott","Ello","EMC","Endo"
        ,"facebook","Feit","fendi","Filings","Fyffes"
        ,"Galleon","Givenchy","goldman","goldmans","google","googled","googles"
            ,"GreeneKing","Groupon"
            ,"Guggenheim"
        ,"Hasbro","herbalife","Hermes","HSBC","HSBCs"
        ,"IBM","ING","intel","Islam"        
        ,"Jacquemus","Jana","Janus","Jefferies","JetBlue","Judaism"
        ,"Kenzo","Kering","knick","Knicks"
        ,"LabCorp","Lazard","Lehman","Lloyds","loeb","Louisiana","luxottica"
        ,"Margiela","Marni","Marriott","merck","Methane","metlife","Microsoft"
        ,"Netflix","NewRepublic","newyorktimes","Nike","Nutreco"
        ,"Oaktree","Ocwen","Oi","okcupid","OnDeck"
        ,"PayPal","PetSmart","Philips","pimco","Prada","PTA","Puma"
        ,"qualcomm"
        ,"RadioShack","Reddit","Repsol","Rodarte"
        ,"Sacai","Salix","Samsung","Santander","Sherborne","SHV","Siemens"
            ,"SoftBank","sony","Soviet","Sovietization","soviets"
            ,"Spotify","Starbucks","Symantec"
        ,"technip","TianheChemicals","Tibco","Travelport","TUI"
        ,"uber","UBM","ubs"
        ,"Valeant","Valeant's","Valentino","Valentino's","Vatican","verizon"
        ,"Walmart","Walmart's","WHotels","WNYC","WNYCs"
        ,"Xerox","Xiaomi","xiaomis"
        ,"yahoo","yahoos","yale","Yara"
                    
        # Product names
        ,"Android","AnonyWatch","ApplePay","botox"
        ,"Celiac","CFDA","Citizenfour","ClassDojo","cosmo"
        ,"Duracell"
        ,"euros","eurozone"
        ,"Hanukkah","hbo","heineken","Hoverboard","Hoverboards"
        ,"instagram","ios","iPhone","iPhones","ipads","ipod","itunes","iWatch"
        ,"Jewish","Jews"
        ,"LACMA","Longpoint"
        ,"medicare","minecraft","MOCA","Muslim","Muslims"
        ,"nazi","nazis","Netrebko","Neverland","nytlnreads"
        ,"obamacare","oculus"
        ,"Pabst","pinterest","Postit"
        ,"Redskins","Reich","REIT","Ruble"
        ,"SABMiller","Showtime","Simpsons","Smartwatch","Snapchat","Soundcheck","Spanx","Synod"
        ,"Technicolor","ts"
        ,"vitamind"
        ,"WhatsApp"
        ,"yankees","youtube"
                    
        # Words not in this SCOWL db
        ,"ABC","ABCs","acrostic","Activism","Adoptees","aeroplanes","ageist","Airbags","Airways"
            ,"alternateside"
            ,"Analytics","Anthropocene","anti","Antidepressants","Antitrust"
            ,"Apologists","app","apps"
            ,"Arent","Artfully","Artsy","artworks"
            ,"Authoritarianism","Autism","Autocorrect"
        ,"Backpedals","Backstory","bailout","Banco","Bartending"
            ,"Beeps","Belle","Benchmark","Benchmarks"
            ,"Bidders","Billionaire","Billionaires","Biotech","Bipartisanship"
                ,"bitcoin","bitcoins"
            ,"bleeping","blower"
            ,"Bohemian","Bohemians","Bona","Bonanza","Borrowers","Bot","Boxy"
            ,"Breaker","breakup","Brewer","Broadband","Broadcaster","brutalism","brutalist"
            ,"Buybacks","Buyout","Buyouts","buzziest"
        ,"cafe","cafes","camerawearing","Cannabis","Canonizes","Carb","Carbs","Catcher"
            ,"Cautionary"
            ,"Cellphone","cellphones","centric","ceos","certifier"
            ,"chairwoman","changer","Chargers","Chaser","Chemotherapy","Childs","Children"
                ,"chokehold","christmas"
            ,"Circusry"
            ,"classsrooms","Closings"
            ,"Co" # Co-<something>; not "Company"
                ,"Colorfully","comity"
                ,"confusingly","congressional","Conservationists","constructively"
                    ,"contractionary","controvert"
                ,"copycat"
                ,"counteroffer","Couture"
            ,"Crackdown","craftsmanship","CrossFit","Crowdfunding"
            ,"cupcakes","Curated","Curates","Curatorial","customizable","cuteness"
            ,"Cyberattack","cyberattacks","Cyberbullies","Cyberbullying","Cybercriminals"
                ,"Cyberdefenses","cybersecurity"
        ,"Dateless","Deflationary","deploy","deployed","deployment","Deporter","derp","derpy"
                ,"Detox","Detoxing"
            ,"Diagramless","Didnt","Disinflation","divorcee"
            ,"dna"
            ,"Dodger","doesnt","dont","Downside","Downsizing"
            ,"Dreamboat"
        ,"ebola","ebolas","EBook","Ebooks","ecigarettes","ECommerce"
            ,"enhancer","Ensnared","Enterovirus","Entrepreneur","entrepreneurial","entrepreneurs"
            ,"Epically","Epidurals","Epiphany","Equalizer","Erasable","ESports","Everyones"
            ,"ex","Exclusivity","Expats","Extravaganza","Extremism"
        ,"F","Fandom","fashionistas","Faux" # synonym with false ???
            ,"Fest","Fides","filmmaker","Flyers","followup","Fondation","Footie","Foreclosures"
            ,"Fracking","Fraudsters","fritz"
        ,"G","Gallerist","Gamers","Geek" ,"Geeks","Gentrification","ghosthunting","Ghoulish"
            ,"Giver"
            ,"glitch","globalization"
            ,"governance","Gown"
            ,"Grandma","grandmasters","Gravedigger","groundbreaking"
        ,"H","halloween","handstands","hashtag","Hashtags","hauntings","Haute","havent","Hawkish"
            ,"Heartbreaking","Helpline","hermitage"
            ,"HipHop"
            ,"Holdings","Holdout","Holdouts"
                ,"homecoming","Homelessness","Homeowners","homestretch","honorees","hospices"
        ,"i","Iconic","Illusionists","Immolation"
            ,"indie","Infighting","influencers","infographics","Innovate","Innovators","insomniac"
                ,"Interfaith","Internet","Introverted"
                ,"Invisibles"
            ,"IPOs"
            ,"Islamic","islamists","isnt"
        ,"Journaling","journalistic","Joyless"
        ,"Kicker","knaves"
        ,"latin","latinas","latinos","Leftover","Leftovers","Legalization","lender","lenders"
            ,"libor","Lifespans","Lineup","liquidity"
            ,"longtime","luddite","Luxe"
        ,"Macroeconomic","Makeover","marathoner","marathoners","Mayoral"
            ,"meatless","meatloaf","Medalists","Mega","meltdowns","messaging"
            ,"Midwest","midterm","midterms","millennial","millennials","Millennium","Millenniums"
                ,"mini","Minivan","mistreatment"
            ,"modernist","Mommy","Moviegoers"
            ,"Multimedia","muslims"
        ,"narcissist","Nemesis","Nerd","Newsroom","newyear","nightcrawler"
            ,"nocturnalist","Nonclassical","Novelistic"
            ,"Nudie","Numberplay"
        ,"offline","olds","Oligarchs","online","onstage","Oops","Orbiter"
        ,"outback","Outreach","Outsells","Outsourcing"
        ,"PAC","PACs","pairings","Payouts","Peacekeeping","Perfumer"
            ,"philharmonic","philharmonics"
            ,"piercings","pizzeria"
            ,"playlist","playlists"
            ,"podcast","Ponytails","Popcast","Populism","Porn","postmodern"
            ,"Pre", "Prenuptial","preps"
                    ,"Preschool","Preschooler","Preschoolers","Preservationist"
                ,"prioritization","prioritizing"
                ,"ProDemocracy","protester","protesters"
        ,"Questioners","quinoa"
        ,"rapper","radicalized","razings"
            ,"Reappointed","Rebooting","reboots","Regroup","regulator","Regulators"
                ,"reimagined","Reinsurance","Reinvent","Reinvents","Reinventing"
                ,"Relievers"
                ,"reshape","Residency","Resurgent","retesting","Retro","Retweet"
            ,"riverbed"
            ,"roadshow","Robo","Robotic","robotics","rodenticide","rollout","Rooftop","roundup"
            ,"runoff"
        ,"salvo","scams","schoolers","screenings","Scrum","Sculptural"
            ,"SelfControl","selfie","Selfies","senatorial"
            ,"shags","shhh","shoemaker","Shootings","shortlists","ShortSeller","shouldnt"
            ,"sightings","Simulcast","Skincare"
            ,"Sliders","slouchy","Slowdown"
            ,"smartphone","smartphones","smoothies","smushing"
            ,"songstress","Soundtracked"
            ,"SpellCheck","Spinoff","spinoffs","sportier","Sporty"
            ,"Standalone","startup","startups","steamer","steelmaker","stepmother","storefront"
                ,"stylist"
            ,"Subpoena","Subpoenas","subprime","Summery","superfan","superfans","supermodels"
            ,"swaths","sweatshirt","Sweeties","Switchers"
        ,"takeovers","Tastings","Taxidermy"
            ,"tech","Technologist","Telecom","tequila","texting"
            ,"thanksgiving","Theatrics","Themed","Therapeutics"
            ,"Timeline","timepiece","timepieces","Titan","Titans"
            ,"torturers","tourism"
            ,"tracker","transportive","treaters","trousseau","trove"
            ,"tshirts","turnabout"
        ,"UniCredit"
        ,"valuation","Vegan","Vegans","Volatility"
        ,"wasnt","Wearable","Wearables","webcasts","Website","Websites","Wellness","werent"
            ,"Whisperer","whitecollar"
            ,"winemaking","Winery","Wireless"
            ,"wmd","wmds"
            ,"Womens","Wonkish","Workday","workups","Wouldnt"
        ))))
#unlist(str_split("second meatless day"," ")) %in% vldWords    
    vldWords <- union(vldWords, glbFeatsTextVldWords)
    ...
    for (txtFeat in glbFeatsText) {
        ...
        print(myprint_df(full_terms_df <-
                get_corpus_terms(txt_corpus=glbFeatsTextCorpus[[txtFeat]], 
                                 compute.chisq = FALSE, compute.nzv = FALSE), 
        ...
        txtTdmNgrams <- get_txt_terms(TdmNgrams,
                                      compute.nzv = FALSE, compute.chisq = FALSE)
        txtTdmNgrams$vldWords <- sapply(1:nrow(txtTdmNgrams), function(rowIx) {
            tokens <- unlist(str_split(txtTdmNgrams[rowIx, "term"], " ")); 
            return(all(tokens %in% vldWords)) })
        invldWords <- subset(txtTdmNgrams, !vldWords)
        print(sprintf("Remaining invalid words: %d", nrow(invldWords)))
        myprint_df(subset(invldWords, select = -term))
        # to id docs that have a specific ngram using txtTdmNgrams[, pos]
        #glbObsAll[TdmNgrams$j[which(TdmNgrams$i == 19349)], glb_dsp_cols]
...
#                                         control=glb_txt_terms_control))
    ...
    require(wordcloud)
    for (txtFeat in glbFeatsText) {
        print(sprintf("    Wordcloud post-stem term weights for %s:", txtFeat))
        ...
        wordcloud(d$word, d$freq, min.freq = minFrq)
    }    
...
#                                             glb_sprs_thresholds[txtFeat])
    ...
    for (txtFeat in glbFeatsText) {
        ...
        terms_sprs_df <- get_txt_terms(sprs_DTM)
...
#stop(here"); glb2Sav(); all.equal(savObsAll, glbObsAll); glbObsAll <- savObsAll
    ...
    for (txtFeat in glbFeatsText) {
        ...
        # terms_full_df <- get_txt_terms(glb_full_DTM_lst[[txtFeat]])     
        terms_full_df <- glb_post_stem_words_terms_df_lst[[txtFeat]]        
...
#         top_val_terms <- orderBy(~-weight, terms_full_df)$term[1:glbFeatsTextTermsMax[[txtFeat]]]
...
#             colnames(txt_X_df) <- paste(txtFeat_pfx, ".T.",
...
#stop(here"); glb2Sav()
    ...
    extract.features_chunk_df <- 
        myadd_chunk(extract.features_chunk_df, paste0("extract.features_", "bind.DXM"), 
                    major.inc = TRUE)
...
#stop(here"); savObsAll <- glbObsAll; glbObsAll <- savObsAll
    ...
    txt_X_df <- foreach(txtFeat = glbFeatsText, .combine = cbind) %do% {           
    # for (txtFeat in glbFeatsText) {
        ...
        txt_X_df[, paste0(txtFeat_pfx, ".wrds.n.log")] <- 
            log(1 + mycount_pattern_occ("\\w+", glb_txt_chr_lst[[txtFeat]]))
        ...
        txt_X_df[, paste0(txtFeat_pfx, ".chrs.n.log")] <- 
            log(1 + mycount_pattern_occ(".", glbObsAll[, txtFeat]))
        txt_X_df[, paste0(txtFeat_pfx, ".chrs.uppr.n.log")] <- 
            log(1 + mycount_pattern_occ("[[:upper:]]", glbObsAll[, txtFeat]))
        txt_X_df[, paste0(txtFeat_pfx, ".dgts.n.log")] <- 
            log(1 + mycount_pattern_occ("[[:digit:]]", glbObsAll[, txtFeat]))
...
#                                     c("UniqueID", "Popular", "Abstract", "A.npnct23.log")]))    
        ...
        txt_X_df[, paste0(txtFeat_pfx, ".chrs.iso8859.n.log")] <- 
            log1p(mycount_pattern_occ("&#[[:digit:]]{3};", glbObsAll[, txtFeat]))
...
#           
            ...
            for (rng in seq(1, length(glb_txt_stop_words[[txtFeat]]), stepVal)) {
                ...
                txt_X_df[, paste0(txtFeat_pfx, ".wrds.stop.n.log")] <-
                    txt_X_df[, paste0(txtFeat_pfx, ".wrds.stop.n.log")] + 
                    mykntpar_pattern_occ(stop_words_rex_str, glb_txt_chr_lst[[txtFeat]])
        ...
        txt_X_df[, paste(txtFeat_pfx, ".P.http.log1p", sep = "")] <- 
            log1p(mycount_pattern_occ("http", glbObsAll[, txtFeat]))
        ...
        for (pattern in names(glbFeatsTextPatterns[[txtFeat]]))
            txt_X_df[, paste(txtFeat_pfx, ".P.", pattern, sep = "")] <- 
                log1p(mycount_pattern_occ(glbFeatsTextPatterns[[txtFeat]][pattern], 
                                            glbObsAll[, txtFeat]))
...
#     print(sapply(names(glbObsNew), function(col) sum(is.na(glbObsNew[, col]))))
    ...
    glbFeatsExclude <- union(glbFeatsExclude, glbFeatsText)
...
if (!is.null(glbFeatsPrice)) {
    extract.features_chunk_df <- 
        myadd_chunk(extract.features_chunk_df, paste0("extract.features_", "xtract.Price.vars"), 
                    major.inc = TRUE)
...
for (txtFeat in glbFeatsText) {
    ...
    print(sprintf("    All post-stem-words term weights for %s:", txtFeat))
    ...
    print(sprintf("    All post-stem-words term freq distribution for %s:", txtFeat))
    print(table(glb_post_stem_words_terms_df_lst[[txtFeat]]$freq))

    print(sprintf("    All post-stem-words term length distribution for %s:", txtFeat))
