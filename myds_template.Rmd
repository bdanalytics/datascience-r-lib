...
# if model should be selected automatically / by default
# mdl_id <- orderBy(glbgetModelSelectFormula(), glb_models_df)[1, "id"]

for (mdlFamily in names(glbMdlPreprocMethods))
    for (mdlSmpMethod in c("repeatedcv"))
        for (mdlAlgMethod in names(glbMdlPreprocMethods[[mdlFamily]])) {
            thsMdlParentId <- myinit_mdl_specs_lst(mdl_specs_lst = list(
                       "id.prefix" = mdlFamily,
                "train.preProcess" = NULL,
             "trainControl.method" = mdlSmpMethod,
                    "train.method" = mdlAlgMethod))$id
            
            indepVar <- trim(unlist(strsplit(glb_models_df[glb_models_df$id == thsMdlParentId,
                                                                  "feats"], "[,]")))
            if (!is.null(glbObsFitOutliers[[mdlFamily]])) {
                fitobs_df <- glbObsFit[!(glbObsFit[, glbFeatsId] %in%
                                                 glbObsFitOutliers[[mdlFamily]]), ]
                    print(sprintf("Outliers removed: %d", nrow(glbObsFit) - nrow(fitobs_df)))
                    print(setdiff(glbObsFit[, glbFeatsId], fitobs_df[, glbFeatsId]))
            } else fitobs_df <- glbObsFit
