...
if (!is.null(glbChunks$first) && (glbChunks$first == "cluster.data") &&
    ...
    myloadChunk(glbChunks$inpFilePathName, keepSpec = c("glbMdlTuneParams","glbMdlEnsemble"),
                dropSpec = c(NULL))
```

## Step ``r mydsp_chunk(glb_chunks_df)``
```{r cluster.data, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c("glbMdlTuneParams","glbMdlEnsemble"), dropSpec = c(NULL))}
...
# if (grep(grp, levels(grp_ids)) != 10) next        
        ...
        #dstns_dist <- proxy::dist(ctgry_allobs_df[, glbFeatsCluster], method = "cosine")
...
if (all(is.na(glbObsNew[, glb_rsp_var]))) {
    ...
    lclgetMatrixSimilarity <- function(x, y) {
        startTm <- proc.time()["elapsed"]
        # print(sprintf("  lclgetMatrixSimilarity: dim(x): %s", toString(dim(x))))
        # print(sprintf("  lclgetMatrixSimilarity: dim(y): %s", toString(dim(y))))        
        yDf <- foreach(obsIx = c(1:nrow(y)), .combine = rbind) %dopar% {
        #yCor <- foreach(obsIx = c(1:nrow(y)), .combine = c) %do% {
        # yCor <- c()
        # for (obsIx in c(1:nrow(y))) {            
            xyDf <- data.frame(cor = mean(proxy::simil(x, y[obsIx, ], method = "correlation"),
                                          na.rm = TRUE),
                         cosineSmy = mean(proxy::simil(x, y[obsIx, ], method = "cosine"),
                                          na.rm = TRUE))
            # print(sprintf("  lclgetMatrixSimilarity: obsIx: %5d; xyCor: %.4f", obsIx, xyCor))
            # xyCor
            # yCor <- c(yCor, xyCor)
        }    
    ...
    # print(lclgetMatrixSimilarity(x = mtxObsFit[1:10, ], y = mtxObsOOB[1:3, ]))
...
#             as.character(rspvar_freq_df[, glb_rsp_var_raw])
        ...
        NewTrnSmy <- lclgetMatrixSimilarity(mtxObsNew, mtxObsTrn)
        NewTrnSmy[, glbFeatsId] <- glbObsTrn[, glbFeatsId]
        NewTrnSmy[, glbFeatsCategory] <- glbObsTrn[, glbFeatsCategory]
        NewTrnSmy[, glb_rsp_var] <- glbObsTrn[, glb_rsp_var]  
        if (rspGrpVar != glb_rsp_var_raw) 
            NewTrnSmy[, rspGrpVar] = tmpObsTrn[, rspGrpVar]
        srtNewTrnSmy <- orderBy(reformulate(c(rspGrpVar, glbFeatsCategory)), NewTrnSmy)        
        thspik <- srtNewTrnSmy$cosineSmy
        
        # When red < blue; this code selects the worst strata to diagnose the problem
        # DO NOT DELETE
        # print(chkIdsDf <- NewOOBSmy %>% dplyr::arrange(cosineSmy) %>% head(5)); print(tmpNewTrnSmy <- glbObsTrn[glbObsTrn[, glbFeatsId] %in% chkIdsDf[, glbFeatsId], c(glbFeatsId, glbFeatsCategory, glb_rsp_var)]); print(tmpNewTrnSmy <- merge(tmpNewTrnSmy, NewTrnSmy, all.x = TRUE)); print(OOBGrd); print(obs_freq_df); print(strataNewTrnSmy <- NewTrnSmy %>% dplyr::filter(Hhold.fctr == "N") %>% dplyr::filter(Party.fctr == "D") %>% arrange(cosineSmy)); strataNewTrnSmy[, "cosineSmyPby"] <- sampling::inclusionprobabilities(strataNewTrnSmy$cosineSmy, 12) # This number is from obs_freq_df[== catgeory, ".strata<glb_rsp_var>"]
        # what happens if the least 5 similarities are hard-coded to 0
        # print(tmppik <- strataNewTrnSmy$cosineSmy); tmppik[1:5] <- 0; print(tmppik)
        # print(sampling::inclusionprobabilities(strataNewTrnSmy$cosineSmy, 12))
        ...
        # set.seed(glbObsTrnPartitionSeed); print(sampling::getdata(strataNewTrnSmy, (splitOrg <- sampling::strata(stratatmp_trn_obs_df, size = 12, pik = strataNewTrnSmy$cosineSmy, method = "systematic", description = TRUE))))
        # set.seed(glbObsTrnPartitionSeed); print(sampling::getdata(strataNewTrnSmy, (splitOrg <- sampling::strata(stratatmp_trn_obs_df, size = 12, pik = tmppik, method = "systematic", description = TRUE))))
    ...
    OOBFitSmy <- lclgetMatrixSimilarity(mtxObsOOB, mtxObsFit)
    OOBNewSmy <- lclgetMatrixSimilarity(mtxObsOOB, mtxObsNew)
    NewOOBSmy <- lclgetMatrixSimilarity(mtxObsNew, mtxObsOOB)
    NewOOBSmy[, glbFeatsId] <- glbObsOOB[, glbFeatsId]
    FitNewSmy <- lclgetMatrixSimilarity(mtxObsFit, mtxObsNew)
    ...
    for (metric in names(smyDf)[!grepl("obs.", names(smyDf), fixed = TRUE)]) {
        ...
        smyThsCtgDf <- as.data.frame(tapply(OOBFitSmy[, metric], glbObsFit[, glbFeatsCategory], 
                                            mean, na.rm = TRUE))
        ...
        smyThsCtgDf <- as.data.frame(tapply(OOBNewSmy[, metric], glbObsNew[, glbFeatsCategory], 
                                            mean, na.rm = TRUE))
        names(smyThsCtgDf) <- metric
        smyThsCtgDf[, glbFeatsCategory] <- row.names(smyThsCtgDf)
        smyThsCtgDf$obs.x = "OOB"
        ...
        smyThsCtgDf <- as.data.frame(tapply(FitNewSmy[, metric], glbObsNew[, glbFeatsCategory], 
                                            mean, na.rm = TRUE))
        names(smyThsCtgDf) <- metric
        smyThsCtgDf[, glbFeatsCategory] <- row.names(smyThsCtgDf)
    ...
    for (metric in setdiff(names(smyDf)[!grepl("obs.", names(smyDf), fixed = TRUE)], "cor"))
        # cor does not seem to be very informative
        stopifnot((smyDf[(smyDf$obs.x == "OOB") & (smyDf$obs.y == "New"), metric] - 
                   smyDf[(smyDf$obs.x == "Fit") & (smyDf$obs.y == "New"), metric]) >= -5.0e-08)
} else {
...
print(glb_models_df[order(glb_models_df$min.elapsedtime.everything), 
                    c("min.elapsedtime.everything"), FALSE])
...
```{r fit.models_2, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c(NULL), dropSpec = c(NULL))}
#myloadChunk(glbChunks$inpFilePathName, keepSpec = c(NULL), dropSpec = c(NULL)); glb2Sav()
...
if (is.null(glbMdlSelId)) 
    glbMdlSelId <- orderBy(glbgetModelSelectFormula(), glb_models_df)[1, "id"] else 
...
if (!all(is.na(glbObsNew[, glb_rsp_var]))) {    
    ...
    mdl_id_pfx <- paste0("Final.", myparseMdlId(glbMdlSelId)$family)
    ...
    thsSpc <- myinit_mdl_specs_lst(mdl_specs_lst = list(
        ...
        train.metric = glbMdlMetricSummary, 
        train.maximize = glbMdlMetricMaximize,    
        train.method = myparseMdlId(glbMdlSelId)$alg,
        train.preProcess = myparseMdlId(glbMdlSelId)$preProcess))
    
    # inherit tune.df from glb_sel_mdl
    thsSpc$tune.df <- cbind(data.frame(mdlId = thsSpc$id), 
                            glb_sel_mdl$bestTune %>% 
...
if (glb_is_classification && glb_is_binomial)
    prob_threshold_def <- 
        glb_models_df[glb_models_df$id == glbMdlSelId, "opt.prob.threshold.OOB"] else
    prob_threshold_def <- NULL
for (obsSet in c("trn", "new")) {
    obs_df <- switch(obsSet, all = glbObsAll, trn = glbObsTrn, new = glbObsNew)
    obs_df <- glb_get_predictions(obs_df, mdl_id = glbMdlFinId, 
                                    rsp_var = glb_rsp_var, 
                                    prob_threshold_def = prob_threshold_def)
    obs_df <- glb_get_predictions(obs_df, mdl_id = glbMdlSelId, 
                                    rsp_var = glb_rsp_var, 
                                    prob_threshold_def = prob_threshold_def)
