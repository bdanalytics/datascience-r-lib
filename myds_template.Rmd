...
if (glb_is_classification & !glb_is_binomial) {
    glb_entity_df[, glb_rsp_var_out] <- predict(glb_sel_mdl, newdata=glb_entity_df, type="raw")
}    
...
if (glb_is_classification & !glb_is_binomial) {
    # Compute selected model predictions
	glb_newent_df[, glb_rsp_var_out] <- predict(glb_sel_mdl, newdata=glb_newent_df, type="raw")
...
if (glb_is_classification & !glb_is_binomial) {
    ...
    print(sprintf("accuracy.sel=%0.4f", sum(diag(newent_conf_mtrx)) * 1.0 / sum(newent_conf_mtrx)))
    
    print(newent_conf_df <- mycreate_xtab(glb_newent_df, 
                                        c(glb_rsp_var, paste0(glb_rsp_var, ".preddmy"))))
    newent_conf_df[is.na(newent_conf_df)] <- 0    
    newent_conf_mtrx <- as.matrix(newent_conf_df[, -1])
    print(sprintf("accuracy.dmy=%0.4f", sum(diag(newent_conf_mtrx)) * 1.0 / sum(newent_conf_mtrx)))    
}    
