...
glbMdlPreprocMethods <- NULL # : default or list(
...
#             as.character(rspvar_freq_df[, glb_rsp_var_raw])
    ...
    lclgetMatrixCorrelation <- function(x, y) {
        startTm <- proc.time()["elapsed"]
        # print(sprintf("  lclgetMatrixCorrelation: dim(x): %s", toString(dim(x))))
        # print(sprintf("  lclgetMatrixCorrelation: dim(y): %s", toString(dim(y))))        
        yCor <- foreach(obsIx = c(1:nrow(y)), .combine = c) %dopar% {
        #yCor <- foreach(obsIx = c(1:nrow(y)), .combine = c) %do% {
        # yCor <- c()
        # for (obsIx in c(1:nrow(y))) {            
            xyCor <- mean(proxy::simil(x, y[obsIx, ], method = "correlation"), na.rm = TRUE)
            # print(sprintf("  lclgetMatrixCorrelation: obsIx: %5d; xyCor: %.4f", obsIx, xyCor))
            # xyCor
            # yCor <- c(yCor, xyCor)
        }    
        print(sprintf("lclgetMatrixCorrelation: duration: %f secs",
                          proc.time()["elapsed"] - startTm))
        return(yCor)
    }
    # print(lclgetMatrixCorrelation(x = mtxObsFit[1:10, ], y = mtxObsOOB[1:3, ]))
    
    mtxObsFit <- lclcastMtx(glbObsFit)
    mtxObsOOB <- lclcastMtx(glbObsOOB)
    mtxObsNew <- lclcastMtx(glbObsNew)    
    
    # lclgetMatrixCorrelation works faster with O(100) obs in each
    # tmpObsFit = mtxObsFit[1:400, ]
    # tmpObsOOB = mtxObsOOB[1:100, ]
    # startTm = proc.time()["elapsed"]
    # print(sprintf("cor of Fit vs. OOB: %.4f",
    #               mean(lclgetMatrixCorrelation(tmpObsFit, tmpObsOOB), na.rm = TRUE)))
    # print(sprintf("lclgetMatrixCorrelation: duration: %f secs",
    #                       proc.time()["elapsed"] - startTm))
    # startTm = proc.time()["elapsed"]    
    ...
    #                                                 na.rm = TRUE)))
    OOBFitCor <- lclgetMatrixCorrelation(mtxObsFit, mtxObsOOB)
    print(sprintf("cor of Fit vs. OOB: %.4f", mean(OOBFitCor, na.rm = TRUE)))
    OOBNewCor <- lclgetMatrixCorrelation(mtxObsNew, mtxObsOOB)
    print(sprintf("cor of New vs. OOB: %.4f", mean(OOBNewCor, na.rm = TRUE)))
    NewFitCor <- lclgetMatrixCorrelation(mtxObsFit, mtxObsNew)
    print(sprintf("cor of Fit vs. New: %.4f", mean(NewFitCor, na.rm = TRUE)))
...
# Low.cor.X
stop("here: Low.cor.X features not implemented correctly")
if (length() {
    fit.models_0_chunk_df <- myadd_chunk(fit.models_0_chunk_df, 
