...
#             txt_X_df[, paste0(txtFeat_pfx, ".wrds.SCOWL.n.log")] <- 0            
#             stepVal <- 500
#             for (rng in seq(1, length(glbSCOWLWords), stepVal)) {
#                 bgn <- rng
#                 end <- bgn + stepVal - 1
#                 if (end > length(glbSCOWLWords))
#                     end <- length(glbSCOWLWords)
#                 print(sprintf("bgn:%d; end:%d", bgn, end))
#                 glbSCOWLWords_rex_str <- paste0("\\b(", 
#                                 paste0(glbSCOWLWords[bgn:end], collapse = "|"),
#                                              ")\\b")
#                 txt_X_df[, paste0(txtFeat_pfx, ".wrds.SCOWL.n.log")] <-
#                     txt_X_df[, paste0(txtFeat_pfx, ".wrds.SCOWL.n.log")] + 
#                     mykntpar_pattern_occ(glbSCOWLWords_rex_str, glb_txt_chr_lst[[txtFeat]])
#                     #log1p(mykntpar_pattern_occ(stop_words_rex_str, glb_txt_chr_lst[[txtFeat]][1]))   
#             }
#             txt_X_df[, paste0(txtFeat_pfx, ".wrds.SCOWL.n.log")] <-
#                     log1p(txt_X_df[, paste0(txtFeat_pfx, ".wrds.SCOWL.n.log")])
         
            mask <- dimnames(glb_post_stem_words_terms_mtrx_lst[[txtFeat]])$Terms %in% glbSCOWLWords
            txt_X_df[, paste0(txtFeat_pfx, ".wrds.SCOWL.n.log")] <-
...
for (txtFeat in names(glbFeatsText)) {
    ...
    print(glbObsAll[
        which(terms_mtrx[, tail(glb_post_stem_words_terms_df_lst[[txtFeat]], 1)$pos] > 0), 
                        c(glb_id_var, names(glbFeatsText))])
