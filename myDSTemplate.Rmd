...
cat(sprintf("Detected %d cores", detectCores()))
registerDoMC(cores = detectCores() - 1) # 1 core reserved for OS
require(foreach)
# foreach(i=1:5) %dopar% sqrt(i)
...
for (new_feat in names(glbFeatsDerive)) {
    print(sprintf("Creating new feature: %s...", new_feat))
    args_lst <- NULL 
    for (arg in glbFeatsDerive[[new_feat]]$args) 
        args_lst[[arg]] <- glbObsAll[, arg]
    names(args_lst) <- paste("raw", seq(1:length(names(args_lst))), sep = "")
...
if (is.null(glbObsTrnFile$splitSpec) || (glbObsTrnFile$splitSpecs$method != "copy")) {
        ...
        for (feat in feats_vctr[((batch-1)*batch_size + 1) : 
            ...
            if (glb_is_regression)
                gp <- myplot_scatter(df = pltObsSmp, ycol_name = glb_rsp_var, xcol_name = feat,
                                     smooth = TRUE)
            if (glb_is_classification)
                gp <- myplot_violin(df = pltObsSmp, ycol_names = feat, xcol_name = glb_rsp_var)
...
### Step ``r mydsp_chunk(glb_chunks_df)``
```{r scrub.data, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c(NULL), dropSpec = c(NULL))}
...
if (!is.null(glbChunks$first) && (glbChunks$first == "cluster.data") &&
    (is.null(knitr::opts_current$get(name = 'label')))) # not knitting
    myloadChunk(glbChunks$inpFilePathName, keepSpec = c(NULL),
                dropSpec = c(NULL))
```

## Step ``r mydsp_chunk(glb_chunks_df)``
```{r cluster.data, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c(NULL), dropSpec = c(NULL))}
if (!is.null(glbFeatsCluster)) {
    #require(hash)
...
#stop(here"); glb2Sav(); glbObsAll <- savObsAll
    ...
    cluster_vars_df <- data.frame(abs.cor.y = abs(cor(
                        data.matrix(glbObsAll[glbObsAll$.src == "Train", glbFeatsCluster]),
            as.numeric(glbObsAll[glbObsAll$.src == "Train", glb_rsp_var]),
                                    use = "pairwise.complete.obs")))
    print(tail(cluster_vars_df <- orderBy(~ abs.cor.y, 
                                    subset(cluster_vars_df, !is.na(abs.cor.y))), 5))
    print(sprintf("    .rnorm abs(cor): %0.4f",
        abs(cor(glbObsAll[glbObsAll$.src == "Train", ".rnorm"], 
            as.numeric(glbObsAll[glbObsAll$.src == "Train", glb_rsp_var]),
            use = "pairwise.complete.obs"))))
    
    if (is.null(glbClusterEntropyVar))
        glbClusterEntropyVar <- glb_rsp_var
    print(sprintf("  Clustering entropy measure: %s", glbClusterEntropyVar))
    
    print(sprintf("glbObsAll Entropy: %0.4f", 
        allobs_ent <- entropy(table(glbObsAll[, glbClusterEntropyVar]),
                              method="ML")))
    
    print(category_df <- mygetCategoryEntropy(obs_df=glbObsAll,
                                             entropy_var=glbClusterEntropyVar,
...
#stop(here"); glb2Sav()    
    ...
    for (grp in grp_ids) {
        ...
        if (!inherits(ctgry_allobs_df[, glbClusterEntropyVar], "factor"))
            ctgry_allobs_df[, glbClusterEntropyVar] <- 
                as.factor(ctgry_allobs_df[, glbClusterEntropyVar])
        
        smyMtx <- mygetMatrixSimilarity(
            x = data.matrix(ctgry_allobs_df[, row.names(cluster_vars_df)]),
                                        weights = cluster_vars_df$abs.cor.y)
        print("  Similarity stats:")
        print(summary(smyMtx))
        smyVar <- sort(sapply(names(smyMtx), 
                                    function(metric) var(smyMtx[metric], na.rm = TRUE)),
                             decreasing = TRUE)
        print("    variance:")
        print(smyVar)
        print(sprintf("selected similarity metric for clustering: %s", 
                      (sltMetric <- names(smyVar)[1])))
        
        # stop("mywgtdcosine output changed from a symmetric matrix to triangular")
        if (sltMetric == "mywgtCosine")
            dstns_dist <- proxy::dist(data.matrix(ctgry_allobs_df[, row.names(cluster_vars_df)]),
                                      method = sltMetric,
                                      weights = cluster_vars_df$abs.cor.y) else
            dstns_dist <- proxy::dist(data.matrix(ctgry_allobs_df[, row.names(cluster_vars_df)]),
                                      method = sltMetric)
        # Custom distance functions return a crossdist object
        if (inherits(dstns_dist, "crossdist")) {
        ...
        } else dstns_mtrx <- as.matrix(dstns_dist)
        
        # maxDist sometimes > 1 ; happens due to NAs ? 
        #      Q99581.fctr Q108342.fctr Q113584.fctr
                      # NA           NA   Technology
                      # No    In-person           NA
        assertthat::see_if(max(dstns_mtrx, na.rm = TRUE) <= 9.9) 
        assertthat::see_if(min(dstns_mtrx, na.rm = TRUE) >= 0.0)        
        if (sum(is.na(dstns_mtrx[, 1])) > 0) {
        #     naObs <- which(is.na(dstns_mtrx[, 1]))
        #     assertthat::are_equal(length(naObs), sum(is.na(dstns_mtrx[, 1])))
        # } else naObs <- c()
            dstns_mtrx[is.na(dstns_mtrx)] <- 10
            dstns_dist <- as.dist(dstns_mtrx)
        }    

        print(sprintf("max distance(%0.4f) pair:", (maxDist <- max(dstns_mtrx, na.rm = TRUE))))
        row_ix <- ceiling(which(dstns_mtrx == maxDist)[1] / ncol(dstns_mtrx))
        col_ix <- which(dstns_mtrx[row_ix, ] == maxDist)
        assertthat::are_equal(maxDist, dstns_mtrx[row_ix, col_ix])
        print(ctgry_allobs_df[c(row_ix, col_ix), 
            c(glbFeatsId, glbClusterEntropyVar, glbFeatsCategory, names(glbFeatsText),
              glbFeatsCluster)])
    
        min_dstns_mtrx <- dstns_mtrx
        diag(min_dstns_mtrx) <- 10  # maxDist sometimes > 1; see above
        # Float representations issue -2.22e-16 vs. 0.0000
        print(sprintf("min distance(%0.4f) pair:", (minDist <- min(min_dstns_mtrx, na.rm = TRUE))))
        row_ix <- ceiling(which(min_dstns_mtrx == minDist)[1] / ncol(min_dstns_mtrx))
        col_ix <- which(min_dstns_mtrx[row_ix, ] == minDist)
        assertthat::are_equal(minDist, dstns_mtrx[row_ix, col_ix])        
        print(ctgry_allobs_df[c(row_ix, col_ix), 
            c(glbFeatsId, glbClusterEntropyVar, glbFeatsCategory, names(glbFeatsText),
              glbFeatsCluster)])
...
#                   lab.col=unclass(ctgry_allobs_df[, glbClusterEntropyVar]))

        opt_minclustersize_df <- data.frame(minclustersize = nrow(ctgry_allobs_df), 
            entropy = entropy(table(ctgry_allobs_df[, glbClusterEntropyVar]),
                              method = "ML"))
        sizeSrch <- as.integer(seq(nrow(ctgry_allobs_df) / 2, nrow(ctgry_allobs_df) / 10, 
                            length = 5))
        sizeSrch <- sizeSrch[sizeSrch > 1]
        for (minclustersize in sizeSrch) {
            ...
            ctgry_clstrs_df <- mygetCategoryEntropy(ctgry_allobs_df,
                                                    glbClusterEntropyVar)
        ...
        clusterGroups <- cutreeDynamic(clusters, minClusterSize = opt_minclustersize,
                                       ...
                                       deepSplit = 0)
        # Unassigned groups are labeled 0; the largest group has label 1
        table(clusterGroups, ctgry_allobs_df[, glbClusterEntropyVar], 
              useNA = "ifany")   
        clusterGroups[clusterGroups == 0] <- 1
        table(clusterGroups, ctgry_allobs_df[, glbClusterEntropyVar], useNA = "ifany") 
    ...
    print(cluster_df <- mygetCategoryEntropy(obs_df=glbObsAll,
                                             entropy_var=glbClusterEntropyVar,
                                             by_var=glbFeatsCategory))
...
#             c(".clusterid.fctr")
    ...
    if (glbFeatsClusterVarsExclude)
        glbFeatsExclude <- c(glbFeatsExclude, glbFeatsCluster)
}
...
rm(dstns_mtrx); rm(min_dstns_mtrx); 
glb_chunks_df <- myadd_chunk(glb_chunks_df, "partition.data.training", major.inc = TRUE)
if (!is.null(glbChunks$first) && (glbChunks$first == "partition.data.training") &&
    (is.null(knitr::opts_current$get(name = 'label')))) # not knitting
    myloadChunk(glbChunks$inpFilePathName, keepSpec = c(NULL), dropSpec = c(NULL))
```

## Step ``r mydsp_chunk(glb_chunks_df)``
```{r partition.data.training, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c(NULL), dropSpec = c(NULL))}
...
if (all(is.na(glbObsNew[, glb_rsp_var]))) {
    ...
    OOB_size <- nrow(glbObsTrn) * 0.2
    
    lclcastMtx <- function(obsDf) {
        #  For char columns convert them into as.numeric(factors)
        #   ignore glbFeatsText for this exercise
    
        #   Find features (ignore glbFeatsExclude; 
        #       assume all column names with period inside them are derived)
        ftr <- setdiff(names(obsDf), 
                        c(glbFeatsId, glb_rsp_var_raw, glb_rsp_var, 
                          grep("\\.", names(obsDf), value = TRUE)))
        ftrChr <- myfind_chr_cols_df(obsDf[, ftr])
        ftrNonChr <- setdiff(ftr, ftrChr)
        if (length(ftrNonChr) == 0) stop("lclCorMtx: this will not work with empty ftrNonChr")
        retDf <- obsDf[, ftrNonChr, FALSE]
            ftrChr <- setdiff(ftrChr, glbFeatsText)
        for (ftr in ftrChr) {
            retDf[, paste0(ftr, ".nmc")] <- as.numeric(as.factor(obsDf[, ftr]))
        }
        
        return(as.matrix(retDf))
    }
    
    if (is.null(glbFeatsCategory)) {
        stop("implement systematic sampling instead of random")
        require(caTools)
        split <- sample.split(glbObsTrn[, glb_rsp_var_raw], 
        ...
        sample_vars <- c(glbFeatsCategory, glb_rsp_var)
        tmpObsTrn <- glbObsTrn[, sample_vars, FALSE]
        tmpObsNew <- glbObsNew[, sample_vars, FALSE]        
        if (!all(is.na(tmpObsNew[, glb_rsp_var])))
            stop("Not implemented yet...")
        if (length(unqValsRspVar <- unique(tmpObsTrn[ ,glb_rsp_var])) > 10) {
            if (!is.numeric(unqValsRspVar))
                stop("Not implemented yet...")
            rspGrpVar <- paste0(glb_rsp_var, ".cut.fctr")
            tmpObsTrn[, rspGrpVar] <- cut(tmpObsTrn[, glb_rsp_var], 
                                          breaks = as.numeric(summary(tmpObsTrn[, glb_rsp_var])))
        } else rspGrpVar <- glb_rsp_var
        ...
        if (sum(is.na(obs_freq_df)) > 0)
            stop("chunk partition.data.training: NAs in obs_freq_df")
        strata_mtrx <- ceiling(
...
#             as.character(rspvar_freq_df[, glb_rsp_var_raw])
        ...
        OOBGrd <- expand.grid(glbFeatsCategory = sort(unique(tmp_trnobs_df[, glbFeatsCategory])), 
                              rspGrpVar = sort(unique(tmp_trnobs_df[, rspGrpVar])))
        names(OOBGrd) <- c(glbFeatsCategory, rspGrpVar)
        OOBGrd[, ".strata"] <- 0
        for (ctg in sort(unique(tmp_trnobs_df[, glbFeatsCategory])))
            for (rspGrp in sort(unique(tmp_trnobs_df[, rspGrpVar])))
                OOBGrd[(OOBGrd[, glbFeatsCategory] == ctg   ) &
                       (OOBGrd[, rspGrpVar       ] == rspGrp) , 
                       ".strata"] <- obs_freq_df[obs_freq_df[, glbFeatsCategory] == ctg, 
                                                 paste0(".strata.", rspGrp)]

        mtxObsTrn <- lclcastMtx(glbObsTrn)
        mtxObsNew <- lclcastMtx(glbObsNew)    
        ftrWgt <- as.vector(abs(cor(mtxObsTrn, as.numeric(glbObsTrn[, glb_rsp_var]), 
                                    use = "pairwise.complete.obs")))
        ftrWgt[is.na(ftrWgt)] <- 0
        TrnNewSmy <- mygetMatrixSimilarity(mtxObsTrn, mtxObsNew, weights = ftrWgt)
        print("TrnNewSimilarity stats:")
        print(summary(TrnNewSmy))
        varTrnNewSmy <- sort(sapply(names(TrnNewSmy), 
                                    function(metric) var(TrnNewSmy[metric], na.rm = TRUE)),
                             decreasing = TRUE)
        print("  variance:")
        print(varTrnNewSmy)
        print(sprintf("selected similarity metric for partitioning: %s", 
                      (sltMetric <- names(varTrnNewSmy)[1])))
        TrnNewSmy[, glbFeatsId] <- glbObsTrn[, glbFeatsId]
        TrnNewSmy[, glbFeatsCategory] <- glbObsTrn[, glbFeatsCategory]
        TrnNewSmy[, glb_rsp_var] <- glbObsTrn[, glb_rsp_var]  
        if (rspGrpVar != glb_rsp_var_raw) 
            TrnNewSmy[, rspGrpVar] = tmpObsTrn[, rspGrpVar]
        srtTrnNewSmy <- orderBy(reformulate(c(rspGrpVar, glbFeatsCategory)), TrnNewSmy)        
        thspik <- srtTrnNewSmy[, sltMetric]
        
        # When red < blue; this code selects the worst strata to diagnose the problem
        # DO NOT DELETE
        # print(chkIdsDf <- NewOOBSmy %>% dplyr::arrange(cosineSmy) %>% head(5)); print(tmpTrnNewSmy <- glbObsTrn[glbObsTrn[, glbFeatsId] %in% chkIdsDf[, glbFeatsId], c(glbFeatsId, glbFeatsCategory, glb_rsp_var)]); print(tmpTrnNewSmy <- merge(tmpTrnNewSmy, TrnNewSmy, all.x = TRUE)); print(OOBGrd); print(obs_freq_df); print(strataTrnNewSmy <- TrnNewSmy %>% dplyr::filter(Hhold.fctr == "N") %>% dplyr::filter(Party.fctr == "D") %>% arrange(cosineSmy)); strataTrnNewSmy[, "cosineSmyPby"] <- sampling::inclusionprobabilities(strataTrnNewSmy$cosineSmy, 12) # This number is from obs_freq_df[== catgeory, ".strata<glb_rsp_var>"]
        # what happens if the least 5 similarities are hard-coded to 0
        # print(tmppik <- strataTrnNewSmy$cosineSmy); tmppik[1:5] <- 0; print(tmppik)
        # print(sampling::inclusionprobabilities(strataTrnNewSmy$cosineSmy, 12))
        # print(sampling::inclusionprobabilities(tmppik, 12))
        # stratatmp_trn_obs_df <- tmp_trnobs_df %>% dplyr::filter(Hhold.fctr == "N") %>% dplyr::filter(Party.fctr == "D"); print(dim(stratatmp_trn_obs_df))
        # set.seed(glbObsTrnPartitionSeed); print(sampling::getdata(strataTrnNewSmy, (splitOrg <- sampling::strata(stratatmp_trn_obs_df, size = 12, pik = strataTrnNewSmy$cosineSmy, method = "systematic", description = TRUE))))
        # set.seed(glbObsTrnPartitionSeed); print(sampling::getdata(strataTrnNewSmy, (splitOrg <- sampling::strata(stratatmp_trn_obs_df, size = 12, pik = tmppik, method = "systematic", description = TRUE))))
        ...
        split_strata <- sampling::strata(tmp_trnobs_df, 
                               ...
                               #size = c(223, 355,  13, 198, 315,  12),
                               # method = "srswor")
                               # pik = rep(1.0, nrow(tmp_trnobs_df)), method = "systematic")
                               pik = thspik, method = "systematic", description = TRUE)        
        # print(table(split_strata$Stratum))
        glbObsOOB <- getdata(tmp_trnobs_df, split_strata)[, names(glbObsTrn)]
        
        # Fix specific strata # Very valuable code for filter_ & interp; DO NOT DELETE
        # strataFix <- list(c(Hhold.fctr = "N", Party.fctr = "D"))
        # for (grp in 1:length(strataFix)) {
        #     orgObsOOB <- glbObsOOB; delObsOOB <- glbObsOOB
        #     fltObsTrn <- tmp_trnobs_df
        #     OOBGrdThs <- OOBGrd
        #     thsTrnNewSmy <- TrnNewSmy
        #     for (cdn in 1:length(strataFix[[grp]])) {
        #         delObsOOB <- delObsOOB %>% 
        #             dplyr::filter_(interp(~(var == strataFix[[grp]][[cdn]]), 
        #                                     var = as.name(names(strataFix[[grp]])[cdn])))
        #         fltObsTrn <- fltObsTrn %>% 
        #             dplyr::filter_(interp(~(var == strataFix[[grp]][[cdn]]), 
        #                                     var = as.name(names(strataFix[[grp]])[cdn])))
        #         OOBGrdThs <- OOBGrdThs %>% 
        #             dplyr::filter_(interp(~(var == strataFix[[grp]][[cdn]]), 
        #                                     var = as.name(names(strataFix[[grp]])[cdn])))
        #         thsTrnNewSmy <- thsTrnNewSmy %>% 
        #             dplyr::filter_(interp(~(var == strataFix[[grp]][[cdn]]), 
        #                                     var = as.name(names(strataFix[[grp]])[cdn])))
        #     }    
        #     glbObsOOB <- glbObsOOB[!(glbObsOOB[, glbFeatsId] %in% 
        #                              delObsOOB[, glbFeatsId]), ]
        #     
        #     strataThs <- sampling::strata(fltObsTrn, 
        #                        # stratanames = c(rspGrpVar, glbFeatsCategory),
        #                        size = OOBGrdThs[, ".strata"],
        #                        pik = thspik, method = "systematic", description = TRUE)
        #     glbObsOOB <- rbind(glbObsOOB,
        #                        getdata(fltObsTrn, strataThs)[, names(fltObsTrn)])
        # }
    ...
    # Check similarity
    mtxObsFit <- lclcastMtx(glbObsFit)
    mtxObsOOB <- lclcastMtx(glbObsOOB)
    OOBFitSmy <- mygetMatrixSimilarity(mtxObsOOB, mtxObsFit, weights = ftrWgt)
    OOBNewSmy <- mygetMatrixSimilarity(mtxObsOOB, mtxObsNew, weights = ftrWgt)
    # OOBNewSmy[, glbFeatsId] <- glbObsOOB[, glbFeatsId]
    FitNewSmy <- mygetMatrixSimilarity(mtxObsFit, mtxObsNew, weights = ftrWgt)
    
    smyDf <-  as.data.frame(rbind(colMeans(OOBFitSmy, na.rm = TRUE),
                                  colMeans(OOBNewSmy, na.rm = TRUE),
                                  colMeans(FitNewSmy, na.rm = TRUE)))
    smyDf$obs.x <- c("OOB", "OOB", "Fit")
    smyDf$obs.y <- c("Fit", "New", "New")
    
    smyCtgDf <- data.frame()
    for (metric in names(smyDf)[!grepl("obs.", names(smyDf), fixed = TRUE)]) {
        smyMtcCtgDf <- data.frame()
        
        smyThsCtgDf <- as.data.frame(tapply(OOBFitSmy[, metric], glbObsOOB[, glbFeatsCategory], 
                                            mean, na.rm = TRUE))
        names(smyThsCtgDf) <- metric
        smyThsCtgDf[, glbFeatsCategory] <- row.names(smyThsCtgDf)
        smyThsCtgDf$obs.x = "OOB"
        smyThsCtgDf$obs.y = "Fit"
        smyMtcCtgDf <- rbind(smyMtcCtgDf, smyThsCtgDf)
        
        smyThsCtgDf <- as.data.frame(tapply(OOBNewSmy[, metric], glbObsOOB[, glbFeatsCategory], 
                                            mean, na.rm = TRUE))
        names(smyThsCtgDf) <- metric
        smyThsCtgDf[, glbFeatsCategory] <- row.names(smyThsCtgDf)
        smyThsCtgDf$obs.x = "OOB"
        smyThsCtgDf$obs.y = "New"
        smyMtcCtgDf <- rbind(smyMtcCtgDf, smyThsCtgDf)
        
        smyThsCtgDf <- as.data.frame(tapply(FitNewSmy[, metric], glbObsFit[, glbFeatsCategory], 
                                            mean, na.rm = TRUE))
        names(smyThsCtgDf) <- metric
        smyThsCtgDf[, glbFeatsCategory] <- row.names(smyThsCtgDf)
        smyThsCtgDf$obs.x = "Fit"
        smyThsCtgDf$obs.y = "New"
        smyMtcCtgDf <- rbind(smyMtcCtgDf, smyThsCtgDf)
        
        if (nrow(smyCtgDf) == 0) smyCtgDf <- smyMtcCtgDf else
            smyCtgDf <- merge(smyCtgDf, smyMtcCtgDf, all = TRUE)
    }    
    
    # plot the similarities by category & metric
    pltCtgDf <- smyCtgDf %>% 
        tidyr::unite(obs.xy, obs.x, obs.y, sep = " vs. ", remove = TRUE) %>%
        tidyr::gather_(key_col = "key", value_col = "value", 
                        gather_cols = names(smyDf)[!grepl("obs.", names(smyDf), fixed = TRUE)]) 
    # %>%
    #     plyr::arrange(obs.xy, key, Hhold.fctr)
    print(ggplot(pltCtgDf, aes_string(x = glbFeatsCategory, y = "value", group = "obs.xy")) + 
              geom_path(aes(color = obs.xy)) + 
              geom_point(aes(color = obs.xy)) + 
              facet_wrap(~key) + 
              ggtitle("Similarity of Partitions\n desired: blue >= red"))
    
    print("Similarity of partitions:"); print(smyDf)
    stopifnot((smyDf[(smyDf$obs.x == "OOB") & (smyDf$obs.y == "New"), sltMetric] - 
               smyDf[(smyDf$obs.x == "Fit") & (smyDf$obs.y == "New"), sltMetric]) >= -5.0e-08)
} else {
...
if (!is.null(glbChunks$first) && (glbChunks$first == "select.features") &&
    (is.null(knitr::opts_current$get(name = 'label')))) # not knitting
    myloadChunk(glbChunks$inpFilePathName, keepSpec = c(NULL), dropSpec = c(NULL))
```

## Step ``r mydsp_chunk(glb_chunks_df)``
```{r select.features, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c(NULL), dropSpec = c(NULL))}
...
tmp_allobs_df <- 
    glbObsAll[, union(setdiff(names(glbObsAll), subset(glb_feats_df, nzv)$id),
                      c(glbFeatsCategory, glbClusterEntropyVar))]
...
pltTrnObs <- glbObsTrn[, c(union(".rnorm",
                        not_nrml_feats_df$id[1:min(5, nrow(not_nrml_feats_df))]),
                            glbClusterEntropyVar), FALSE]
if (is.null(glbClusterEntropyVar))
    gp <- myplot_violin(df = pltTrnObs, ycol_names = names(pltTrnObs)) else
    gp <- myplot_violin(pltTrnObs, 
                    setdiff(names(pltTrnObs), glbClusterEntropyVar), 
                    xcol_name = glbClusterEntropyVar)
...
if (!is.null(glbChunks$first) && (glbChunks$first == "fit.models_0") &&
    (is.null(knitr::opts_current$get(name = 'label')))) # not knitting
    myloadChunk(glbChunks$inpFilePathName, keepSpec = c(NULL), dropSpec = c(NULL))
```

## Step ``r mydsp_chunk(glb_chunks_df)``
```{r fit.models_0, cache=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c(NULL), dropSpec = c(NULL))}
...
```{r fit.models_1, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c(NULL), dropSpec = c(NULL))}

# to save models temporarily; File needs to be manually moved to data directory; renamed & specified as inputFile in glbChunks
# sav_models_df <- glb_models_df; savObjects <- setdiff(grep("glb", ls(envir = globalenv()), value = TRUE),c(NULL, "glbChunks")); print("Largest 5 objects:"); print(tail(sort(sapply(savObjects, function(obj) object.size(get(obj, envir = .GlobalEnv)))), 5)); save(list = savObjects, file = paste0("latest_", tail(glb_models_df$id, 1), ".RData"))

# To deal with parallel process crashes
# glbMdlSequential <- union(glbMdlSequential, "All.X#zv.pca#rcv#glmnet"); print(glbMdlSequential)

# To tune models; If best model contains pca, tune the second best first & then the best 
# mdlId="All.X##rcv#glmnet"
# mdlId="All.X#nzv#rcv#glmnet"
# mdlId="All.X#zv.pca#rcv#glmnet"
# ggplot(glb_models_lst[[mdlId]], highlight = TRUE); print(glb_models_lst[[mdlId]]$bestTune)
# print(sort(unique(glb_models_lst[[mdlId]]$results$alpha)))
# print(sort(unique(glb_models_lst[[mdlId]]$results$lambda)))
# thsTuneParams <- rbind(data.frame() ,data.frame(parameter = "alpha",  vals = "0.100 0.325 0.550 0.775 1.000") ,data.frame(parameter = "lambda", vals = "1.847495e-02 0.02 0.03296959 0.04 0.05")) #  max.Accuracy.OOB = 0.7927461 @ 1 0.01847495
#                                                0.7875648 @ 0.775 0.03296959
# glbMdlTuneParams <- rbind(data.frame(), cbind(data.frame(mdlId = mdlId), thsTuneParams)); print(glbMdlTuneParams)
# glb2Sav(); print(nrow(sav_models_df)); glb_models_df <- sav_models_df[sav_models_df$id != mdlId, ]; print(nrow(glb_models_df))
# ctrl-L; shft-cmnd-retn
# after params finalized move to r set_global_options chunk

# to add preProc models 
#   Impute methods are useful only if there are NAs in the data fed into models
# glbMdlPreprocMethods[["All.X"]] <- list("glmnet" = setdiff(pkgPreprocMethods, c("knnImpute", "bagImpute", "medianImpute"))); print(glbMdlPreprocMethods)
    #                                 # c(NULL)))
    #                                 c("zv.YeoJohnson.pca")))
...
if ((any(grepl("RFE.X", names(glbMdlFamilies), fixed = TRUE))) &&
    is.null(glbRFEResults)) {
    glbRFEResults <- myrun_rfe(obs_df = glbObsFit, indepVar = mygetIndepVar(glb_feats_df), 
...
# for (mdl_id_pfx in names(glbMdlFamilies)[2]) {    
    fit.models_1_chunk_df <- 
        myadd_chunk(fit.models_1_chunk_df, paste0("fit.models_1_", mdl_id_pfx),
                    major.inc = FALSE, label.minor = "setup")

    indepVar <- NULL; topindep_var <- NULL; interact_vars <- NULL; 
    thsPreproc <- NULL;

    if (grepl("\\.Inc", mdl_id_pfx)) {
        if (is.null(topindep_var) && is.null(interact_vars)) {
        #   select best root#!pca#?#? model upto now
            dsp_models_df <- orderBy(model_sel_frmla <- glbgetModelSelectFormula(),
                                     glb_models_df)
            thsRootFamily <- gsub(".Inc", "", mdl_id_pfx, fixed = TRUE)
            dsp_models_df <- dsp_models_df[sapply(dsp_models_df$id, 
                                    function(id) myparseMdlId(id)$family == thsRootFamily), ]
            dsp_models_df <- dsp_models_df[sapply(dsp_models_df$id, 
                                    function(id) !(grepl("pca", myparseMdlId(id)$preProcess))), ]
            # dsp_models_df <- dsp_models_df[sapply(dsp_models_df$id, 
            #                             function(id) myparseMdlId(id)$alg    == "glmnet"     ), ]
            # dsp_models_df <- subset(dsp_models_df, 
            #                         (myparseMdlId(id)$family == thsRootFamily) & 
            #                         (myparseMdlId(id)$alg    == "glmnet"))
            stopifnot(nrow(dsp_models_df) > 0)
            bst_mdl_id <- dsp_models_df$id[1]
            stopifnot(!is.null(glb_models_lst[[bst_mdl_id]]))
            thsPreproc <- myparseMdlId(bst_mdl_id)$preProcess
            # mdl_id_pfx <- 
        ...
        #   select important features
            if (is.null(bst_featsimp_df <- 
                ...
                warning("Root model for ", mdl_id_pfx, " : ", bst_mdl_id, 
            ...
            while (is.null(topindep_var) && (topindep_ix <= nrow(bst_featsimp_df))) {
                topindep_var <- gsub("`", "", row.names(bst_featsimp_df)[topindep_ix])
    ...
    if (grepl("\\.Inc", mdl_id_pfx)) mdl_methods <- myparseMdlId(bst_mdl_id)$alg else
    if (is.null(glbMdlFamilies[[mdl_id_pfx]]))
...
# Base Model for preProc should be selected automatically for each family
for (family in setdiff(names(glbMdlPreprocMethods), c(""))) {
    bstMdlDf <- orderBy(glbgetModelSelectFormula(), glb_models_df)
    bstMdlDf <- bstMdlDf[sapply(bstMdlDf$id, function(mdlId) myparseMdlId(mdlId)$family == family),
                         ]
    # print(head(bstMdlDf[, c("max.Accuracy.OOB"), FALSE], 20))
    bstMdlId <- bstMdlDf[1, "id"]
    bstMdlIdComponents <- myparseMdlId(bstMdlId)
    
    # if (bstMdlIdComponents$preProcess != "") 
    #     next # best model already has non-null preprocessing

    indepVar <- trim(unlist(strsplit(glb_models_df[glb_models_df$id == bstMdlId,
                                                   "feats"], "[,]")))
    if (!is.null(glbObsFitOutliers[[bstMdlIdComponents$family]])) {
        fitobs_df <- glbObsFit[!(glbObsFit[, glbFeatsId] %in%
                                     glbObsFitOutliers[[bstMdlIdComponents$family]]), ]
        print(sprintf("Outliers removed: %d", nrow(glbObsFit) - nrow(fitobs_df)))
        print(setdiff(glbObsFit[, glbFeatsId], fitobs_df[, glbFeatsId]))
    } else fitobs_df <- glbObsFit

    for (mdlPrepr in 
         glbMdlPreprocMethods[[bstMdlIdComponents$family]][[bstMdlIdComponents$alg]]) {
        ret_lst <- myfit_mdl(mdl_specs_lst = myinit_mdl_specs_lst(mdl_specs_lst = list(
            id.prefix = bstMdlIdComponents$family,
            type = glb_model_type, tune.df = glbMdlTuneParams,
            trainControl.method = bstMdlIdComponents$resample,
            trainControl.number = glb_rcv_n_folds,
            trainControl.repeats = glb_rcv_n_repeats,
            trainControl.classProbs = glb_is_classification,
            trainControl.summaryFunction = glbMdlMetricSummaryFn,
            ...
            train.metric = glbMdlMetricSummary,
            train.maximize = glbMdlMetricMaximize,
            train.method = bstMdlIdComponents$alg,
            train.preProcess = mdlPrepr)),
            indepVar = indepVar, rsp_var = glb_rsp_var,
            fit_df = fitobs_df, OOB_df = glbObsOOB)
    }
}
...
print(head(orderBy(glbgetModelSelectFormula(), glb_models_df)[, 
    glbMdlMetricsEval[glbMdlMetricsEval %in% names(glb_models_df)], FALSE], 10))
...
fit.models_1_chunk_df <- 
    myadd_chunk(fit.models_1_chunk_df, "fit.models_1_end", major.inc = FALSE,
                label.minor = "teardown")
glb_chunks_df <- myadd_chunk(glb_chunks_df, "fit.models", major.inc = FALSE)
if (!is.null(glbChunks$first) && (glbChunks$first == "fit.models_2") &&
    (is.null(knitr::opts_current$get(name = 'label')))) # not knitting
    myloadChunk(glbChunks$inpFilePathName, 
                keepSpec = c("glbMdlPreprocMethods", "glbMdlTuneParams", "glbMdlEnsemble"), 
                dropSpec = c(NULL))
```

```{r fit.models_2, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c("glbMdlPreprocMethods", "glbMdlTuneParams", "glbMdlEnsemble"), dropSpec = c(NULL))}
...
if (!is.null(glbMdlEnsemble)) {
    ...
    if (glbMdlEnsemble == "auto") {
        glbMdlEnsemble <- mygetEnsembleAutoMdlIds()
        mdl_id_pfx <- paste0(mdl_id_pfx, ".auto")        
    } else if (grepl("^%<d-%", glbMdlEnsemble)) {
        glbMdlEnsemble <- eval(parse(text =
                        str_trim(unlist(strsplit(glbMdlEnsemble, "%<d-%"))[2])))
    }
    
    for (mdl_id in glbMdlEnsemble) {
        ...
        mdlObsTrn <- read.csv(paste0("predictions/", mdl_id, "_Trn.csv"), header = TRUE)
        
        mdlObsFit <- merge(glbObsFit, mdlObsTrn, by = glbFeatsId, all.x = TRUE)
        stopifnot(nrow(mdlObsFit) == nrow(glbObsFit))
        stopifnot(identical(setdiff(names(mdlObsFit), names(glbObsFit)),
                            setdiff(names(mdlObsTrn), glbFeatsId)))
        glbObsFit <- mdlObsFit
        
        mdlObsOOB <- merge(glbObsOOB, mdlObsTrn, by = glbFeatsId, all.x = TRUE)
        stopifnot(nrow(mdlObsOOB) == nrow(glbObsOOB))
        stopifnot(identical(setdiff(names(mdlObsOOB), names(glbObsOOB)),
                            setdiff(names(mdlObsTrn), glbFeatsId)))
        glbObsOOB <- mdlObsOOB
        
        glbObsTrn <- merge(glbObsTrn, mdlObsTrn, by = glbFeatsId, all.x = TRUE)
    }
    
    ensIndepVar <- gsub("#", ".", 
                     paste(mygetPredictIds(glb_rsp_var)$value, glbMdlEnsemble, sep = ""),
                     fixed = TRUE)
    if (glb_is_classification)
        ensIndepVar <- paste(ensIndepVar, ".prob", sep = "")
...
#         indepVar <- indepVar[!grepl("err$", indepVar)]
    ...
    # Some models in glbMdlEnsemble might not be fitted e.g. RFE.X.Interact
    # indepVar <- intersect(indepVar, names(glbObsFit))
    
    # Filter models that have (relatively) high correlation(s)
    mdlEnsembleDf <- myselect_features(entity_df = glbObsTrn[,
                                                c(glbFeatsId, glb_rsp_var, ensIndepVar, ".rnorm")],
                                  exclude_vars_as_features = c(glbFeatsId),
                                  rsp_var = glb_rsp_var)
    mdlEnsembleDf <- orderBy(~-cor.y,
                            myfind_cor_features(feats_df = mdlEnsembleDf,
                                obs_df = glbObsTrn[, 
                                                c(glbFeatsId, glb_rsp_var, ensIndepVar, ".rnorm")],
                                                rsp_var = glb_rsp_var,
                                                nzv.freqCut = glbFeatsNzvFreqMax,
                                                nzv.uniqueCut = glbFeatsNzvUniqMin))
    ensIndepVar <- mygetIndepVar(mdlEnsembleDf)
    # ensIndepVar <- intersect(ensIndepVar, 
    #                          setdiff(union(setdiff(unique(mdlEnsembleDf$cor.high.X), c(NA)),
    #                                        mdlEnsembleDf[is.na(mdlEnsembleDf$cor.high.X), "id"]),
    #                                  glbFeatsId))
    bstMdlDf <- orderBy(glbgetModelSelectFormula(), glb_models_df)
    # print(head(bstMdlDf, 5))
    bstMdlId <- bstMdlDf[1, "id"]
    bstIndepVar <- grep(gsub("#", ".", bstMdlId, fixed = TRUE), 
                        mygetIndepVar(mdlEnsembleDf), fixed = TRUE, value = TRUE)
    stopifnot(bstIndepVar %in% ensIndepVar)
    # if (!(bstIndepVar %in% ensIndepVar)) {
    #     repIndepVar <- mdlEnsembleDf[!is.na(mdlEnsembleDf$cor.high.X) & 
    #                                  (mdlEnsembleDf$cor.high.X == bstIndepVar), "id"]    
    #     ensIndepVar <- union(setdiff(ensIndepVar, ))
    # }

    stopifnot(length(ensIndepVar) > 0)

    # Conduct a step-wise search thru the filtered models
    thsIndepVar <- c(bstIndepVar, ".rnorm")
    for (mdlIx in 1:length(setdiff(ensIndepVar, ".rnorm"))) {
        # print("thsEnsembleId:"); print(thsEnsembleId)
        # thsIndepVar <- sapply(thsEnsembleId, function(mdlId) 
        #     grep(gsub("#", ".", mdlId, fixed = TRUE), ensIndepVar, fixed = TRUE, value = TRUE))
        # thsIndepVar <- c(thsIndepVar, ".rnorm")
        thsMdlSpecs <- myinit_mdl_specs_lst(mdl_specs_lst = list(
                id.prefix = paste0("Ensemble.auto.", mdlIx),
        ...
        ret_lst <- myfit_mdl(mdl_specs_lst = thsMdlSpecs,
            indepVar = thsIndepVar, rsp_var = glb_rsp_var,
            fit_df = glbObsFit, OOB_df = glbObsOOB)
        
        print(varImp(glb_models_lst[[thsMdlSpecs$id]]))
        
        # Pick the next least correlated model 
        print(corRes <- as.data.frame(cor(glbObsTrn[, setdiff(thsIndepVar, ".rnorm"),
                                                    FALSE],
                                          glbObsTrn[, setdiff(ensIndepVar, ".rnorm"), FALSE])))
        print(nxtIndepVar <- sort(colMeans(corRes), decreasing = TRUE))
        nxtIndepVar <- nxtIndepVar[!(names(nxtIndepVar) %in% thsIndepVar)]
        nxtIndepVar <- tail(nxtIndepVar, 1)
        thsIndepVar <- union(thsIndepVar, names(nxtIndepVar))
    }
    
    # For the "best" model check if sampling methods improve prediction performance
    bstMdlId <- orderBy(glbgetModelSelectFormula(), glb_models_df)[1, "id"]
    for (sampleMthd in glbMdlEnsembleSampleMethods) {
        thsMdlSpecs <- myinit_mdl_specs_lst(mdl_specs_lst = list(
                id.prefix = myparseMdlId(bstMdlId)$family,
                type = glb_model_type, tune.df = NULL,
                trainControl.method = sampleMthd,
                trainControl.number = glb_rcv_n_folds,
                trainControl.repeats = glb_rcv_n_repeats,
                trainControl.classProbs = glb_is_classification,
                trainControl.summaryFunction = glbMdlMetricSummaryFn,
    ...
    # plot the results of the Ensemble models
    bstMdlId <- orderBy(glbgetModelSelectFormula(), glb_models_df)[1, "id"]    
    pltMdlDf <- subset(glb_models_df, grepl("Ensemble", id))
    pltMdlDf$nModels <- sapply(pltMdlDf$feats, function(feats) 
                                                length(unlist(strsplit(feats, ","))) - 1)
    pltMdlDf$sampleMethod <- sapply(pltMdlDf$id, function(mdlId) myparseMdlId(mdlId)$resample)
    print(mypltModelStats(df = pltMdlDf, 
                          measure = setdiff(glbMdlMetricsEval, "min.elapsedtime.everything"), 
                          dim = c("nModels", "sampleMethod"),
                          highLightIx = which(pltMdlDf$id == bstMdlId),
                          title = "Ensemble.auto", fileName = NULL))
    print(mypltModelStats(df = pltMdlDf, measure = "min.elapsedtime.everything", 
                          dim = c("nModels", "sampleMethod"),
                          highLightIx = which(pltMdlDf$id == bstMdlId),
                          title = "Ensemble.auto", fileName = NULL))
...
if (is.null(glbMdlSltId)) 
    glbMdlSltId <- orderBy(glbgetModelSelectFormula(), glb_models_df)[1, "id"] else 
    print(sprintf("User specified selection: %s", glbMdlSltId))   
    
# knitr crashes sometimes with plot.glmnet
if (!(is.null(knitr::opts_current$get(name = 'label'))) &&
    (myparseMdlId(glbMdlSltId)$alg == "glmnet"))
    print(glbMdlSlt <- glb_models_lst[[glbMdlSltId]]) else
    myprint_mdl(glbMdlSlt <- glb_models_lst[[glbMdlSltId]])
 
# From here to save(), this should all be in one function
#   these are executed in the same seq twice more:
#       fit.data.training & predict.data.new chunks
print(sprintf("%s fit prediction diagnostics:", glbMdlSltId))
glbObsFit <- glb_get_predictions(df = glbObsFit, mdl_id = glbMdlSltId, 
                                 rsp_var = glb_rsp_var)
print(sprintf("%s OOB prediction diagnostics:", glbMdlSltId))
glbObsOOB <- glb_get_predictions(df = glbObsOOB, mdl_id = glbMdlSltId, 
                                     rsp_var = glb_rsp_var)

print(glb_featsimp_df <- myget_feats_importance(mdl = glbMdlSlt, featsimp_df = NULL))
...
glb_analytics_diag_plots <- function(obs_df, mdl_id, prob_threshold=NULL) {
    ...
    if (glb_is_regression) {
        if (is.null(featsimp_df) || (nrow(featsimp_df) == 0))
            warning("No important features in glbMdlFnlNsl") else
...
if (glb_is_classification && glb_is_binomial)
    glb_analytics_diag_plots(obs_df = glbObsOOB, mdl_id = glbMdlSltId, 
...
if (!is.null(glbFeatsCategory)) {
    glbLvlCategory <- merge(glbLvlCategory, 
            myget_category_stats(obs_df = glbObsFit, mdl_id = glbMdlSltId, 
                                 label = "fit"), 
                            by = glbFeatsCategory, all = TRUE)
    row.names(glbLvlCategory) <- glbLvlCategory[, glbFeatsCategory]
    glbLvlCategory <- merge(glbLvlCategory, 
            myget_category_stats(obs_df = glbObsOOB, mdl_id = glbMdlSltId,
                                 label="OOB"),
...
write.csv(glbObsOOB[, c(glbFeatsId, 
    ...
    paste0(gsub(".", "_", paste0(glbOut$pfx, glbMdlSltId), fixed=TRUE), 
...
## Step ``r mydsp_chunk(glb_chunks_df)``
```{r fit.data.training_0, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c(NULL), dropSpec = c(NULL))}

# mdlId = glbMdlFnlNslId
# mdlId = glbMdlFnlRslId
# ggplot(glb_models_lst[[mdlId]], highlight = TRUE); print(glb_models_lst[[mdlId]]$bestTune)
# print(sort(unique(glb_models_lst[[mdlId]]$results$alpha)))
# print(sort(unique(glb_models_lst[[mdlId]]$results$lambda)))
# thsTuneParams <- rbind(data.frame() ,data.frame(parameter = "alpha",  vals = "0.100 0.325 0.550 0.775 1.000") ,data.frame(parameter = "lambda", vals = "1.847495e-02 0.02 0.03296959 0.04 0.05")) #  max.Accuracy.OOB = 0.7927461 @ 1 0.01847495
...
if (!is.null(glbMdlFnlNslId) && (glbMdlFnlNslId %in% names(glb_models_lst))) {
    print(sprintf("Final non-sampled model: %s is user-specified", glbMdlFnlNslId))
    glbMdlFnlRslId <- glbMdlFnlNslId
    glbMdlFnlNsl <- glbMdlFnlRsl <- glb_models_lst[[glbMdlFnlNslId]]
} else 
if (!all(is.na(glbObsNew[, glb_rsp_var]))) {    
    warning("Final model same as glbMdlSltId")
    glbMdlFnlRslId <- glbMdlFnlNslId <- paste0("Trn.", glbMdlSltId)
    glbMdlFnlNsl <- glbMdlSlt
    glb_models_lst[[glbMdlFnlNslId]] <- glbMdlFnlNsl
    mdlDf <- glb_models_df[glb_models_df$id == glbMdlSltId, ]
    mdlDf$id <- glbMdlFnlNslId
    glb_models_df <- rbind(glb_models_df, mdlDf)
} else {    
    if (myparseMdlId(glbMdlSltId)$family == "RFE.X") {
        indepVar <- mygetIndepVar(glb_feats_df)
        trnRFEResults <- 
    ...
    if (grepl("Ensemble", glbMdlSltId)) {
        # Find which models are relevant
        mdlimp_df <- subset(myget_feats_importance(glbMdlSlt), imp > 5)
    ...
    # "Final" model
    if ((model_method <- glbMdlSlt$method) == "custom")
        # get actual method from the mdl_id
        model_method <- tail(unlist(strsplit(glbMdlSltId, "[.]")), 1)
    ...
    #                                fixed = TRUE), glbMdlSltId)) != -1))
    #     ths_preProcess <- str_sub(glbMdlSltId, match_pos, 
    #                             match_pos + attr(match_pos, "match.length") - 1) else
    #     ths_preProcess <- NULL   
    
    # mdl_id_pfx <- ifelse(grepl("Ensemble", glbMdlSltId),
    #                                "Trn.Ensemble", "Final")
    # thsMdlId <- paste0("Trn.", glbMdlSltId)
    # thsMdlIdComponents <- myparseMdlId(thsMdlId)
    # # mdl_id_pfx <- paste("Final", myparseMdlId(glbMdlSltId)$family, sep = ".")
    mdl_id_pfx <- paste0("Trn.", myparseMdlId(glbMdlSltId)$family)
    ...
    # Force fitting of Trn.glm to identify outliers
    # method_vctr <- unique(c(myparseMdlId(glbMdlSltId)$alg, glbMdlFamilies[["Final"]]))
        
    thsNslSpc <- myinit_mdl_specs_lst(mdl_specs_lst = list(
        id.prefix = mdl_id_pfx, 
        type = glb_model_type, 
        trainControl.method = "none",
        # trainControl.number = 1,
        # trainControl.repeats = 1,
        trainControl.classProbs = glb_is_classification,
        trainControl.summaryFunction = glbMdlMetricSummaryFn,
        trainControl.blockParallel = glbMdlSequential,
        train.metric = glbMdlMetricSummary, 
        train.maximize = glbMdlMetricMaximize,    
        train.method = myparseMdlId(glbMdlSltId)$alg,
        train.preProcess = myparseMdlId(glbMdlSltId)$preProcess))
    
    # inherit tune.df from glbMdlSlt
    thsNslSpc$tune.df <- cbind(data.frame(mdlId = thsNslSpc$id), 
                            glbMdlSlt$bestTune %>% 
                               tidyr::gather(key = parameter, value = value) %>% 
                                dplyr::mutate(vals = as.character(value)))

    glbMdlFnlNslId <- thsNslSpc$id
    
    thsRslSpc <- myinit_mdl_specs_lst(mdl_specs_lst = list(
        id.prefix = mdl_id_pfx, 
        type = glb_model_type, 
        tune.df = glbMdlTuneParams,
        trainControl.method = myparseMdlId(glbMdlSltId)$resample,
        trainControl.number = glb_rcv_n_folds,
        trainControl.repeats = glb_rcv_n_repeats,
        trainControl.classProbs = glb_is_classification,
        trainControl.summaryFunction = glbMdlMetricSummaryFn,
    ...
    glbMdlFnlRslId <- thsRslSpc$id
    
    if (!(grepl("Ensemble", glbMdlSltId))) {
        ret_lst <- myfit_mdl(mdl_specs_lst = thsNslSpc,
                             indepVar = mdlIndepVar,
                             rsp_var = glb_rsp_var, 
                             fit_df =  trnobs_df, OOB_df = NULL) 
        
        if (glbMdlFnlRslId != glbMdlFnlNslId)
            ret_lst <- myfit_mdl(mdl_specs_lst = thsRslSpc,
                                 indepVar = mdlIndepVar,
                                 rsp_var = glb_rsp_var, 
                                 fit_df =  trnobs_df, OOB_df = NULL) 
    } else {
        stop("Ensemble models: resampling yes or no ?")                         
                                 
        # Final model same as selected model except for the model features
        tmp_models_df <- glb_models_df[glb_models_df$id == glbMdlSltId, ]
        tmp_models_df$id <- paste0("Trn.", tmp_models_df$id)
        row.names(tmp_models_df) <- tmp_models_df$id
        ...
        # 
        glb_models_lst[[glbMdlFnlNslId]] <- tmp_fin_mdl
    }
        
    glbMdlFnlNsl <- glb_models_lst[[glbMdlFnlNslId]] 
}
...
if (glb_is_classification && glb_is_binomial)
    prob_threshold_def <- 
        glb_models_df[glb_models_df$id == glbMdlSltId, "opt.prob.threshold.OOB"] else
    prob_threshold_def <- NULL
for (obsSet in c("trn", "new")) {
    obs_df <- switch(obsSet, all = glbObsAll, trn = glbObsTrn, new = glbObsNew)
    obs_df <- glb_get_predictions(obs_df, mdl_id = glbMdlFnlNslId, 
                                    rsp_var = glb_rsp_var, 
                                    prob_threshold_def = prob_threshold_def)
    obs_df <- glb_get_predictions(obs_df, mdl_id = glbMdlFnlRslId, 
                                    rsp_var = glb_rsp_var, 
                                    prob_threshold_def = prob_threshold_def)
    obs_df <- glb_get_predictions(obs_df, mdl_id = glbMdlSltId, 
                                    rsp_var = glb_rsp_var, 
                                    prob_threshold_def = prob_threshold_def)
...
glb_analytics_diag_plots(obs_df = glbObsNew, mdl_id = glbMdlFinId, 
...
require(stringr)
obsOutFinDf <- obs_df[, glbFeatsId, FALSE]
for (clmn in names(glbObsOut$vars))
    if (!grepl("^%<d-%", glbObsOut$vars[[clmn]]))
        obsOutFinDf[, clmn] <- obs_df[, glbObsOut$vars[[clmn]]] else {
            feat <- str_trim(unlist(strsplit(glbObsOut$vars[[clmn]], "%<d-%"))[2])
            obsOutFinDf[, clmn] <- eval(parse(text = feat))
    }        

obsOutSelDf <- obs_df[, glbFeatsId, FALSE]
for (clmn in names(glbObsOut$vars))
    if (!grepl("^%<d-%", glbObsOut$vars[[clmn]]))
        obsOutSelDf[, clmn] <- obs_df[, glbObsOut$vars[[clmn]]] else {
            feat <- str_trim(unlist(strsplit(glbObsOut$vars[[clmn]], "%<d-%"))[2])
            feat <- gsub("glbMdlFinId", "glbMdlSelId", feat)
            obsOutSelDf[, clmn] <- eval(parse(text = feat))
    }        

if (glb_is_classification) {
    rsp_var_out <- paste0(mygetPredictIds(glb_rsp_var)$value, glbMdlFinId)
    if (".grpid" %in% names(glbObsNew)) {
        ...
        tmp_newobs_df <- 
            merge(tmp_newobs_df, obsOutFinDf, by = glbFeatsId, all.x = TRUE)
        tmp_newobs_df$.err <- 
    ...
    predctId <- mygetPredictIds(glb_rsp_var, glbMdlFinId)$value
    NewLvlCategory <- 
...
#predctId <- mygetPredictIds(glb_rsp_var, glbMdlFinId)$value; errObsNew <- glbObsNew[(glbObsNew[, glbFeatsCategory] %in% tmpLvlCategory[, 1]), ];
#myprint_df(errObsNew[(errObsNew[, glbFeatsCategory] %in% tmpLvlCategory[errLvlIx, glbFeatsCategory]) & (errObsNew[, predctId] == "Y"), union(c(glbFeatsId, glbFeatsCategory, predctId), myextract_actual_feats(row.names(glb_featsimp_df[glb_featsimp_df[, paste0(glbMdlFinId, ".imp")] > 10, ])))])
#myprint_df(errObsNew[(errObsNew[, glbFeatsCategory] %in% "Foreign#World#") & (errObsNew[, predctId] == "Y"), union(c(glbFeatsId, glbFeatsCategory, predctId), myextract_actual_feats(row.names(glb_featsimp_df[glb_featsimp_df[, paste0(glbMdlFinId, ".imp")] > 10, ])))])
    }
...
#stop(here")    
    ...
    ranges_all_df <- glbObsAll[, tmp_feats_df$id, FALSE] %>% 
    ...
    if (nrow(tmp_feats_df) == 1) {
        # For some reason dplyr::summarise_each does not prefix result with id
        ranges_all_df$id <- tmp_feats_df$id
    }
...
#     sav_ranges_new_df <- ranges_new_df; all.equal(sav_ranges_new_df, ranges_new_df)    
    ...
    rsp_var_out_OOB <- mygetPredictIds(glb_rsp_var, glbMdlSelId)$value
    rsp_var_out_new <- mygetPredictIds(glb_rsp_var, glbMdlFinId)$value
...
#stop(here"); glb2Sav(); sav_obsOutFinDf <- obsOutFinDf; all.equal(sav_obsOutFinDf, obsOutFinDf); obsOutFinDf <- sav_obsOutFinDf

# This does not work for classification since AUC distribution might be different for different models
#   -> Run glm on .prob from this glbMdlFinId & .prob from stacked file & stack condition as a feature
if (!is.null(glbOutStackFnames)) {
    for (fname in glbOutStackFnames) {
        print(sprintf("Stacking file %s to prediction outputs...", fname))
        #obsOutFinDf <- dplyr::arrange_(rbind(obsOutFinDf, read.csv(fname)), "UniqueID")
        obsOutFinDf <- dplyr::arrange_(rbind(obsOutFinDf, 
                #read.csv(fname) %>% filter(!(UniqueID %in% obsOutFinDf$UniqueID))),
            #read.csv(fname) %>% filter(!(UniqueID %in% obsOutFinDf[, glbFeatsId]))),
            read.csv(fname) %>% 
                dplyr::filter_(interp(~!(var %in% obsOutFinDf$var), 
                                      var = as.name(glbFeatsId)))),
                                    glbFeatsId)
        
        if (nrow(obsOutFinDf) != length(unique(obsOutFinDf[, glbFeatsId])))
            stop("Potential dups in stacked prediction output")
        
        obsOutSelDf <- dplyr::arrange_(rbind(obsOutSelDf, 
                read.csv(fname) %>% 
                dplyr::filter_(interp(~!(var %in% obsOutSelDf$var), 
                                      var = as.name(glbFeatsId)))),
                                    glbFeatsId)
        
        if (nrow(obsOutSelDf) != length(unique(obsOutSelDf[, glbFeatsId])))
            stop("Potential dups in stacked prediction output")
    }
}

#stop(here"); 
# savobsOutFinDf <- obsOutFinDf; all.equal(savobsOutFinDf, obsOutFinDf)
if (!is.null(glbObsOut$mapFn)) {
    obsOutFinDf <- glbObsOut$mapFn(obsOutFinDf)
    obsOutSelDf <- glbObsOut$mapFn(obsOutSelDf)    
}    
#print(head(obsOutFinDf))

# out_fname <- paste0(glbOut$pfx, "out.csv")
write.csv(obsOutFinDf, paste0(glbOut$pfx, "out_fin.csv"), quote = FALSE, row.names = FALSE)
write.csv(obsOutSelDf, paste0(glbOut$pfx, "out_sel.csv"), quote = FALSE, row.names = FALSE)
...
if (glb_is_classification && glb_is_binomial)
    print(glb_models_df[glb_models_df$id == glbMdlSelId, 
                        "opt.prob.threshold.OOB"])
print(sprintf("glbMdlSelId: %s", glbMdlSelId))
print(sprintf("glbMdlFinId: %s", glbMdlFinId))
print(subset(glbgetDisplayModelsDf(), select = -id))

if (glb_is_regression) {
    print(sprintf("%s OOB RMSE: %0.4f", glbMdlSelId,
            glb_models_df[glb_models_df$id == glbMdlSelId, "min.RMSE.OOB"]))
}    

if (glb_is_classification) {
    print(sprintf("%s OOB confusion matrix & accuracy: ", glbMdlSelId))
    print(t(confusionMatrix(glbObsOOB[, mygetPredictIds(glb_rsp_var, glbMdlSelId)$value],
                            glbObsOOB[, glb_rsp_var])$table))
}            

if (!is.null(glbFeatsCategory)) {
    glbLvlCategory <- merge(glbLvlCategory, 
                            myget_category_stats(obs_df = glbObsTrn, mdl_id = glbMdlFinId, 
                                                 label = "trn"),
                            by = glbFeatsCategory, all = TRUE)
    row.names(glbLvlCategory) <- glbLvlCategory[, glbFeatsCategory]
    
    glbLvlCategory <- merge(glbLvlCategory, 
                            myget_category_stats(obs_df = glbObsNew, mdl_id = glbMdlFinId,
                                                 label = "new"),
                            by = glbFeatsCategory, all = TRUE)
    row.names(glbLvlCategory) <- glbLvlCategory[, glbFeatsCategory]
...
if ((glb_rsp_var %in% names(glbObsNew)) &&
    ...
    if (glb_is_regression) {    
        pred_stats_df <- 
            mypredict_mdl(mdl = glb_models_lst[[glbMdlFinId]], 
                          df = glbObsNew, 
                          rsp_var = glb_rsp_var, 
                          label = "new",
                model_summaryFunction = glb_sel_mdl$control$summaryFunction,
                          ...
                          ret_type = "stats")        
        print(sprintf("%s prediction stats for glbObsNew:", glbMdlFinId))
        print(pred_stats_df)
    } else {   
        print(sprintf("%s new confusion matrix & accuracy: ", glbMdlFinId))
        print(t(confusionMatrix(glbObsNew[, mygetPredictIds(glb_rsp_var, glbMdlFinId)$value],
...
print(orderBy(as.formula(paste0("~ -", gsub("#", ".", glbMdlSelId), ".imp")), 
              subset(tmpFeatsImp, imp.max > 10, select = -imp.max)))

dsp_myCategory_conf_mtrx <- function(myCategory) {
    print(sprintf("%s OOB::myCategory=%s confusion matrix & accuracy: ", 
                  glbMdlSelId, myCategory))
    print(t(confusionMatrix(
        glbObsOOB[glbObsOOB$myCategory == myCategory, 
                      paste0(mygetPredictIds(glb_rsp_var)$value, glbMdlSelId)], 
    ...
    print(sprintf("%s OOB::myCategory=%s FN errors: %d", glbMdlSelId, myCategory,
                  nrow(OOB_FNerr_df)))
    print(OOB_FNerr_df)
    ...
    OOB_FPerr_df <- glbObsOOB[(glbObsOOB$UniqueID %in% err_ids) & 
                        ...
                        c(
                            ".clusterid", 
                            "Popular", "Headline", "Snippet", "Abstract")]
    print(sprintf("%s OOB::myCategory=%s FP errors: %d", glbMdlSelId, myCategory,
                  nrow(OOB_FPerr_df)))
    print(OOB_FPerr_df)
}
...
if (glb_is_regression)
    print(myplot_histogram(glbObsNew, mygetPredictIds(glb_rsp_var, glbMdlFinId)$value))
if (glb_is_classification)
    print(table(glbObsNew[, mygetPredictIds(glb_rsp_var, glbMdlFinId)$value]))
...
print(sprintf("Detected %d cores", detectCores()))
